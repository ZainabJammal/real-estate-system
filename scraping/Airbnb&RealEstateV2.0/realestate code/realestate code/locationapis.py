import requests
import json
import csv
import time

# Define the URL and headers
url = f"https://www.realestate.com.lb/_next/data/tJD_gYeW-5efE5YO9Cx7J/en/real-estate-brokers-agents-lebanon/1225.json?id=1225&_={int(time.time())}"

headers = {
    "Accept": "*/*",
    "Accept-Language": "en-US,en;q=0.9,ar;q=0.8",
    "Connection": "keep-alive",
    "Cookie": "_ga=GA1.1.693996357.1736874612; _gcl_au=1.1.1554251831.1736874612; _ga_HRCEDE2K8L=GS1.1.1738247956.12.1.1738249323.58.0.247075379; XSRF-TOKEN=%3D; realestatecomlb_session=%3D",
    "Referer": "https://www.realestate.com.lb/en/buy-apartment-house-lebanon/el-metn-district/apartment/CP-AK05",
    "Sec-Fetch-Dest": "empty",
    "Sec-Fetch-Mode": "cors",
    "Sec-Fetch-Site": "same-origin",
    "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/132.0.0.0 Safari/537.36",
    "purpose": "prefetch",
    "sec-ch-ua": '"Not A(Brand";v="8", "Chromium";v="132", "Google Chrome";v="132"',
    "sec-ch-ua-mobile": "?0",
    "sec-ch-ua-platform": '"Windows"',
    "x-nextjs-data": "1",
}

try:
    # Make the GET request
    response = requests.get(url, headers=headers)
    print(f"Status Code: {response.status_code}")  # Debugging: Check the status code
    print(f"Response Content: {response.text[:500]}")  # Debugging: Print the first 500 characters of the response

    response.raise_for_status()  # Raise an exception for HTTP errors

    # Try to parse the response as JSON
    try:
        data = response.json()
    except json.JSONDecodeError:
        print("Server returned non-JSON content:")
        print(response.text)
        exit()

    # Extract the locations data
    locations = data.get("pageProps", {}).get("locations", [])
    if not locations:
        raise ValueError("No locations data found in the response.")

    # Define CSV file path
    csv_file = "locations_data.csv"

    # Define CSV column headers
    fieldnames = [
        "id", "province_id", "name_en", "name_ar", "copy_field_en", "copy_field_ar"
    ]

    # Write data to CSV
    with open(csv_file, mode="w", newline="", encoding="utf-8") as file:
        writer = csv.DictWriter(file, fieldnames=fieldnames)
        writer.writeheader()  # Write the header row

        # Write each location to the CSV file
        for location in locations:
            row = {
                "id": location.get("id", ""),
                "province_id": location.get("province_id", ""),
                "name_en": location.get("name_en", ""),
                "name_ar": location.get("name_ar", ""),
                "copy_field_en": location.get("copy_field_en", ""),
                "copy_field_ar": location.get("copy_field_ar", ""),
            }
            writer.writerow(row)

    print(f"Locations data successfully written to {csv_file}")

except requests.exceptions.RequestException as e:
    print(f"Request failed: {e}")
except ValueError as e:
    print(f"Data extraction error: {e}")
except Exception as e:
    print(f"An error occurred: {e}")
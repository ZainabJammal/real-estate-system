// TimeSeriesForecasting.jsx (Updated for 5-Year Forecasting) -> Renamed to CurrentPriceEstimation.jsx
import { useQuery } from '@tanstack/react-query';
// import { Line } from 'react-chartjs-2'; // Not used in this version
import React, {useEffect, useState } from "react";

// Chart.js imports are not needed for this price estimation component
// import {
//   Chart as ChartJS,
//   CategoryScale,
//   LinearScale,
...
const fetchPropertyInputOptions = async () => {
  ...
  const response = await fetch('/api/property_input_options'); // CORRECTED ENDPOINT
  if (!response.ok) {
...
const CurrentPriceEstimation = () => {
  ...
  const [selectedProvince, setSelectedProvince] = useState(""); // ADDED
  ...
  useEffect(() => { // UNCOMMENTED
    if (options) {
      ...
      if (!selectedCity && options.cities?.length > 0) setSelectedCity(options.cities[0]);
      if (!selectedDistrict && options.districts?.length > 0) setSelectedDistrict(options.districts[0]);
      if (!selectedProvince && options.provinces?.length > 0) setSelectedProvince(options.provinces[0]);
  ...
  }, [options, selectedCity, selectedDistrict, selectedProvince, selectedType, selectedBedrooms, selectedBathrooms, sizeM2]);
  ...
  const handleSubmit = async (e) => {
    ...
    const params = {
      ...
      Province: selectedProvince, // ADDED
      Type: selectedType,
    ...
    <div className="price-estimation-container" style={{ padding: '20px', fontFamily: 'Arial, sans-serif' }}>
      ...
      <form onSubmit={handleSubmit} style={{ marginBottom: '20px', padding: '15px', border: '1px solid #ccc', borderRadius: '5px' }}>
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '20px' }}>
          {renderSelect("selectedCity", selectedCity, setSelectedCity, "City", options.cities || [], "city")}
        ...
        <button
          type="submit"
          disabled={isLoadingPrice || !selectedCity || !selectedDistrict || !selectedProvince || !selectedType } // Basic validation
          style={{ marginTop: '20px', padding: '10px 20px', cursor: (isLoadingPrice || !selectedCity || !selectedDistrict || !selectedProvince || !selectedType) ? 'not-allowed' : 'pointer' }}
        >

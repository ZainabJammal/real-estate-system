...
const PriceEstimator = () => {
  ...
  .then(response => {
    ...
    if (!response.ok) {
        // This will catch the 500 errors and other issues
        throw new Error(`Network response was not ok, status: ${response.status}`);
    }
    return response.json();
})
.then(data => {
    // 'data' is the JSON object from your Quart backend
    // It looks like: { status: "success", predicted_price_$: 123000, district: "Beirut" }
    
    console.log("Success:", data);

    // Now you can set the state to display the price
    // e.g., setPredictedPrice(data.predicted_price_$);
})
.catch(error => {
    // This will catch the NameError crash and any network failures
  ...
  <div className="estimator-layout">
      <div className="estimator-content">
        <div className="estimator-title" style={{ fontSize: '20px !important' }}>

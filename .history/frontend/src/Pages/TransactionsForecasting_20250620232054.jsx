...
const TransactionsForecasting = () => {
  // --- CHANGED ---: Use single, clean city names that your API expects.
  // The backend will map these to regions. This makes the frontend much simpler.
  const predefinedCities = [
    "Beirut", "Tripoli", "Akkar", "Baabda", "Aley", "Chouf", "Kesrouan", "Jbeil", "Metn", "Saida", "Zahle"
  ];
  const [selectedCity, setSelectedCity] = useState("");
  const [granularity, setGranularity] = useState('M');

  const fetchForecast = async (city) => {
    // The API call itself remains the same, which is great.
  ...
  // --- CHANGED ---: Now we destructure the NEW keys from the API response.
  if (forecastData) {
    const { historical_data: historical, monthly_forecast: monthly, yearly_forecast: yearly, region_forecasted } = forecastData;
    
    // The rest of the data processing logic is fine because the nested keys ('date', 'transaction_value', etc.) are the same.
    if (granularity === 'M') {
  ...
  const chartOptions = {
    ...
    plugins: { 
      legend: { position: 'top' }, 
      // --- CHANGED ---: The chart title is now dynamic and more informative.
      title: { 
        display: true, 
        text: forecastData 
          ? `Transaction Value Forecast for ${forecastData.region_forecasted}` 
        ...
        <div className="dashboard-components">
          <div className="form-card">
            ...
            <form onSubmit={handleSubmit}>
              <div className="form-group">
                ...
                {/* --- CHANGED ---: The dropdown now uses the new, clean list of cities. */}
                <select id="selectedCity" value={selectedCity} onChange={(e) => setSelectedCity(e.target.value)}>
                  <option value="">Select a City</option>
                  {predefinedCities.sort().map((city) => <option key={city} value={city}>{city}</option>)}
                </select>
              </div>
              <div className="form-group">
          ...
          {forecastData && !isLoading && !isFetching && (
            <div className="chart-card">
              {/* --- CHANGED ---: Add a subtitle to inform the user about the aggregation. This is KEY for user experience. */}
              <div className="chart-subtitle">
                {forecastData.city_requested !== forecastData.region_forecasted
                  ? `Showing aggregated data for the ${forecastData.region_forecasted} region.`
                  : `Showing data for the ${forecastData.region_forecasted} region.`
                }
              </div>

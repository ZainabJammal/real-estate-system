...
import { Line } from 'react-chartjs-2';
import React, { useState } from "react";
import "./TransactionsForecasting.css";
...
const predefinedCities = [
  { key: "Baabda", displayName: "Baabda, Aley, Chouf" },
  { key: "Beirut", displayName: "Beirut" },
  { key: "Bekaa", displayName: "Bekaa" },
  { key: "Kesrouan", displayName: "Kesrouan, Jbeil" },
  { key: "tripoli", displayName: "Tripoli, Akkar" },
];
...
const TransactionsForecasting = () => {
  ...
  // Your fetch function remains the same
  const fetchForecast = async (city) => {
    // Ensure you are using the correct port for your backend
    const response = await fetch(`http://127.0.0.1:8000/forecast/xgboost/${city}`);
    if (!response.ok) {
      const errorData = await response.json().catch(() => ({ error: 'Failed to parse error response' }));
      throw new Error(errorData.error || `HTTP error! status: ${response.status}`);
  ...
  const {
    ...
    queryKey: ['forecast', selectedCityKey],
    queryFn: () => fetchForecast(selectedCityKey),
    enabled: false,
    retry: false,
  });
  ...
  // --- *** THE FIX IS HERE: Robust Chart Data Processing *** ---
  
  // Start with an empty structure
  const chartData = { labels: [], datasets: [] };
  
  // Only try to build the chart if we have valid data
  if (forecastData && forecastData.monthly_forecast) {
    // Provide default empty arrays to prevent crashes if a key is missing
    ...
    if (granularity === 'M') {
      ...
      chartData.datasets.push({
        label: 'Historical', 
        data: [...historical.map(h => h.transaction_value), ...new Array(monthly.length).fill(null)],
        borderColor: 'rgb(54, 162, 235)', 
        backgroundColor: 'rgba(54, 162, 235, 0.5)',
      });
      chartData.datasets.push({
        label: 'Forecast', 
        data: [...new Array(historical.length).fill(null), ...monthly.map(f => f.predicted_value)],
        borderColor: 'rgb(255, 99, 132)', 
        backgroundColor: 'rgba(255, 99, 132, 0.5)', 
        borderDash: [5, 5],
      });
    } else { // Yearly granularity
      const yearlyHist = historical.reduce((acc, item) => {
        const year = new Date(item.date).getFullYear();
        acc[year] = (acc[year] || 0) + item.transaction_value;
        return acc;
      ...
      chartData.datasets.push({
        ...
        data: allYrs.map(y => yearlyHist[y] || null),
        borderColor: 'rgb(54, 162, 235)', 
        backgroundColor: 'rgba(54, 162, 235, 0.5)',
      });
      chartData.datasets.push({
        label: 'Forecast Total', 
        data: allYrs.map(y => { const f = yearly.find(i => i.year === y); return f ? f.total_value : null; }),
        borderColor: 'rgb(255, 99, 132)', 
        backgroundColor: 'rgba(255, 99, 132, 0.5)', 
        borderDash: [5, 5],
      });
  ...
  const chartOptions = { /* ... your options remain the same ... */ };

  return (
    <div className="forecasting-layout">
      <div className="forecasting-content">
        <div className="forecasting-title">
          <h1>Transaction Value Forecasting</h1>
        </div>
        ...
        <div className="dashboard-components">
          <div className="form-card">
            <form onSubmit={handleSubmit}>
              ...
              <div className="form-group">
                <label htmlFor="selectedCity">City</label>
                <select id="selectedCity" value={selectedCityKey} onChange={(e) => setSelectedCityKey(e.target.value)}>
                  <option value=""> Select a City</option>
                  {predefinedCities.map((city) => <option key={city.key} value={city.key}>{city.displayName}</option>)}
                </select>
              </div>
              <div className="form-group">
                <label htmlFor="granularity">View By</label>
              ...
              <div className="form-group">
                <button type="submit" disabled={isFetching || !selectedCityKey}>
          ...
          {isFetching && <p className="status-message">Loading forecast...</p>}
          {error && <p className="error-message">Error: {error.message}</p>}
          
          {/* This conditional rendering is now much safer */}
          {forecastData && !isFetching && !error && (
            <div className="chart-card">
              <div className="chart-wrapper">
                <Line options={chartOptions} data={chartData} />
              </div>

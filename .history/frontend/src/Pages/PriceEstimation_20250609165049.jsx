...
const CurrentPriceEstimation = () => {
  ...
  useEffect(() => { 
    if (options) {
      ...
      if (!selectedBedrooms && options.bedroom_options?.length > 0) setSelectedBedrooms(String(options.bedroom_options?.sort((a,b) => a - b)[0].map(String))); // Sort to get the lowest number
  ...
  return (
    <div className="price-estimation-container" style={{ padding: '20px', fontFamily: 'Arial, sans-serif', width: '100%', maxWidth: '800px' }}>
      <h2>Current Property Price Estimation</h2>
      <form onSubmit={handleSubmit} style={{ marginTop:'25px',marginBottom: '20px', padding: '15px', maxWidth: '100%', border: '1px solid #ccc', borderRadius: '5px' }}>
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '50px' }}>
          {renderSelect("selectedDistrict", selectedDistrict, setSelectedDistrict, "District", options.districts || [], "district", { flex: '1 1 200px' })}
          {renderSelect("selectedType", selectedType, setSelectedType, "Type", options.types || [], "type", { flex: '1 1 200px' })}

          <div className="form-group" style={{ marginBottom: '10px' }}>
            <label htmlFor="sizeM2" style={{ marginTop: '10px', marginLeft: '10px', display: 'block' }}>Size (m²): {sizeM2}</label>
            <input
              type="range"
              id="sizeM2"
              min={options.size_range?.min ?? 0} // Use ?? for default if min/max are undefined
              max={options.size_range?.max ?? 500}
              value={sizeM2}
        ...
        <div style={{ display: 'flex', alignContent: 'flex', marginTop: '' }}>
          <div>
              {selectedType !== "Land" && (
                <div style={{ paddingTop: '10px', display: "flex", gap: "50px " }}>

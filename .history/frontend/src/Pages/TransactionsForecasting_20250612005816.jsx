...
const TimeSeriesForecasting = () => {
  ...
  return (
    <div className="prediction-container" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '20px', backgroundColor: 'var(--background-light-white-p)', boxShadow: '0px 1px 5px 1px var(--background-light-gray-p)', borderRadius: '5px', width: '80%', margin: 'auto' }}>
      ...
      <section><form onSubmit={handleSubmit} style={{ flex: '1', display: 'flex', borderRadius:'5px', border: '1px solid var(--background-light-gray-f)',
          fontSize:'20px'}}>
        <div className="form-group">
          <label htmlFor="selectedCity" style={{ paddingLeft: '10px', fontSize: '20px', fontFamily:'Cantarell, "Open Sans", "Helvetica Neue", sans-serif' }}>City:</label>
          <select
            id="selectedCity"
            value={selectedCity}
            onChange={(e) => setSelectedCity(e.target.value)}
          >
            {/* FIX: Changed the text for the empty value option */}
        ...
        <button 
          ...
          style={{ padding: '8px 15px', cursor: (isLoading || isFetching || !selectedCity) ? 'not-allowed' : 'pointer' }}
        >
      ...
      {predictionData?.forecast && !isFetching && (
        <div className="results" style={{ marginTop: '20px', flex: '1', display: 'flex', borderRadius:'5px', border: '1px solid var(--background-light-gray-f)', backgroundColor: 'var(--background-light-white-p)',
          boxShadow: '0px 1px 5px 1px var(--background-light-gray-p)', boxSizing: 'border-box', transition: 'transform 0.225s ease-in-out' }}>

...
const TransactionsForecasting = () => {
  ...
  if (forecastData) {
    ...
    if (granularity === 'M') {
      ...
      // 2. Mean Forecast (The main prediction line)
      chartData.datasets.push({
        label: 'Mean Forecast',
        // --- FIX 1: Use 'mean_forecast' instead of 'predicted_value' ---
        data: [...new Array(historical.length).fill(null), ...monthly.map(f => f.mean_forecast)],
        borderColor: 'rgb(255, 99, 132)',
        backgroundColor: 'rgba(255, 99, 132, 0.5)',
        borderDash: [5, 5],
      });

      // 3. (BONUS) P10 and P90 for Confidence Interval
      chartData.datasets.push({
          label: '80% Confidence Interval',
          data: [...new Array(historical.length).fill(null), ...monthly.map(f => f.p90_forecast)],
          borderColor: 'rgba(150, 150, 150, 0.2)',
          pointRadius: 0,
          fill: '+1' // Fill to the next dataset in the list (the P10)
      });
       chartData.datasets.push({
          label: 'P10', // Label doesn't matter, it won't be shown
          data: [...new Array(historical.length).fill(null), ...monthly.map(f => f.p10_forecast)],
          borderColor: 'rgba(150, 150, 150, 0.2)',
      ...
      chartData.datasets.push({
        ...
        // --- FIX 2: Use 'mean_forecast' instead of 'total_value' ---
        data: allYrs.map(y => {
          const f = yearly.find(i => i.year === y);
          return f ? f.mean_forecast : null;
        }),
        borderColor: 'rgb(255, 99, 132)',
        backgroundColor: 'rgba(255, 99, 132, 0.5)',
        borderDash: [5, 5],

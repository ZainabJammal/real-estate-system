...
if __name__ == "__main__":
    ...
    train_for_tuning = pd.get_dummies(train_set_raw, columns=['city'], prefix='city').dropna()
    FEATURES = [col for col in train_for_tuning.columns if col != TARGET_COL]
    X_train_tune = train_for_tuning[FEATURES]
    y_train_tune = train_for_tuning[TARGET_COL]
    ...
    X_validation = validation_final.reindex(columns=FEATURES, fill_value=0).dropna()
    y_validation = validation_final.loc[X_validation.index, TARGET_COL]
    ...
    city_to_plot = 'Baabda, Aley, Chouf' # You can change this to any original city name
    print(f"\n-> Generating validation plot for: {city_to_plot}")
    
    sanitized_city_to_plot = sanitize_city_name(city_to_plot)

    train_city = train_set_raw[train_set_raw['city'] == sanitized_city_to_plot].sort_index()
    validation_city = validation_set_raw[validation_set_raw['city'] == sanitized_city_to_plot].sort_index()

    # Create a Series of predictions with the correct date index
    predictions_series = pd.Series(validation_predictions, index=y_validation.index)
    # Filter this series for the specific city we want to plot
    city_plot_predictions = predictions_series[predictions_series.index.isin(validation_city.index)].sort_index()

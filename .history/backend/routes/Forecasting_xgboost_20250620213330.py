...
def sanitize_city_name(name):
    """Replaces problematic characters with underscores for clean column names."""
    return re.sub(r'[ ,/]', '_', name)

def load_and_preprocess_data(filepath):
    """Loads, cleans, and sanitizes the raw data."""
    print("-> Loading and preprocessing data...")
    df = pd.read_csv(filepath)
    parts = df['date'].str.split('-', expand=True)
    df['date_str'] = '01-' + parts[1] + '-' + parts[0]
...
def tune_hyperparameters(X, y):
    """Tunes hyperparameters using GridSearchCV and returns the best parameters."""
    param_grid = {
        'n_estimators': [500, 1000],
        'max_depth': [3, 5],
        'learning_rate': [0.01, 0.05],
    ...
    best_hyperparams = tune_hyperparameters(
        train_for_tuning_df[FEATURES], 
        train_for_tuning_df[TARGET_COL]
    )

    # 3. VALIDATION
    print("\n-> Training optimized model on 2011-2015 data for validation...")
    X_train_final = train_for_tuning_df[FEATURES]
    y_train_final = train_for_tuning_df[TARGET_COL]
    
    optimized_model = xgb.XGBRegressor(objective='reg:squarederror', random_state=42, **best_hyperparams)
    ...
    train_for_tuning_df = pd.get_dummies(train_set_raw, columns=['city'], prefix='city').dropna()
    FEATURES = [col for col in train_for_tuning_df.columns if col != TARGET_COL]
    ...
    # First, create the full feature set for the validation data
    validation_final = pd.get_dummies(validation_set_raw, columns=['city'], prefix='city')
    # Then, drop rows with NaNs. This creates our clean validation set.
    validation_clean = validation_final.dropna()
    
    # Now, create X and y FROM THE SAME CLEANED DATAFRAME to guarantee they match.
    X_validation = validation_clean.reindex(columns=FEATURES, fill_value=0)
    y_validation = validation_clean[TARGET_COL]
    production_model = xgb.XGBRegressor(objective='reg:squarederror', random_state=42, **best_hyperparams)
    production_model.fit(X_all, y_all)
    ...
    plot_validation_results(
        ...
        city_to_plot='Baabda_Aley_Chouf' # Use the sanitized name here now
    )

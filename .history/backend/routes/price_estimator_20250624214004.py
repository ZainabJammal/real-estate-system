...
def train_and_evaluate_model():
    """
    Loads, cleans, processes, trains, and evaluates the property price prediction model.
    """
    # --- 1. Load Data ---
    ...
    # This is a critical step to ensure data integrity before training.
    print("\nStep 2: Cleaning mistaken zero values...")
    types_with_rooms = ['Apartment', 'House/Villa', 'Chalet', 'Office', 'Residential Building']

    # Identify and mark "impossible" zero bedrooms for replacement
    bedrooms_mistake_condition = (
        (df['type'].isin(types_with_rooms)) &
        (df['bedrooms'] == 0) &
        (df['size_m2'] > 35) # A property over 35m² is unlikely to be a studio
    )
    ...
    # Identify and mark "impossible" zero bathrooms
    bathrooms_mistake_condition = (
        (df['type'].isin(types_with_rooms)) &
        (df['bathrooms'] == 0) &
        (df['size_m2'] > 40) # A property over 40m² is unlikely to have zero bathrooms
    )
    ...
    # Convert to integer as they should be whole numbers
    ...
    # Remove the top 1% of prices and sizes to create a more stable model
    ...
    # city is redundant with district. lat/lon add complexity we'll omit for now.
    ...
    # These are solid starting parameters. Tuning is discussed below.
    lgbm_regressor = lgb.LGBMRegressor(
        objective='regression_l1', # MAE is more robust to price outliers than MSE
        n_estimators=1500,
        learning_rate=0.03,
        num_leaves=31,
        max_depth=-1,
    ...
    print(f"  - R²: The model explains {r2:.1%} of the variance in property prices.")
    print(f"  - MAE: On average, the model's price prediction is off by ${mae:,.2f}.")
    print("  - RMSE: Similar to MAE but gives a higher penalty to large errors.")
...
if __name__ == '__main__':
    ...
    print(f"Predicted Price for new property: ${predicted_price[0]:,.2f}")
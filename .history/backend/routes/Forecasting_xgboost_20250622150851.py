...
#     region_to_plot = 'beirut' 
#     print(f"-> Generating validation plot for a single region: '{region_to_plot}'")

#     # Isolate the original (non-encoded) data for plotting the historical and actual values
#     train_region_df = data_with_features[
#         (data_with_features.index < split_date) & 
#         (data_with_features[GROUPING_KEY] == region_to_plot)
#     ]
#     validation_region_df = data_with_features[
#         (data_with_features.index >= split_date) & 
#         (data_with_features[GROUPING_KEY] == region_to_plot)
#     ]
...
#     X_validation_region = X_test[X_test[f'{GROUPING_KEY}_{region_to_plot}'] == 1]

#     if not X_validation_region.empty:
#         # Predict ONLY on this region's data. This guarantees we get exactly 12 predictions.
#         region_plot_predictions = model.predict(X_validation_region)

#         # Call the plotting function you defined at the top of the script
#         plot_validation_results(
#             train_df=train_region_df,
#             validation_df=validation_region_df,
#             predictions_df=region_plot_predictions,
#             city_name=region_to_plot.capitalize()  # Pass the name for the title
#         )
#     else:
#         print(f"No validation data found to plot for region '{region_to_plot}'.")

# # We also need to slightly modify the plotting function to accept the new 'city_name' argument

# # --- End of new validation/plotting block ---
...
#         objective='reg:squarederror',
#         n_estimators=1000, # You can use the full number here
#         learning_rate=0.05,
#         max_depth=7,
#         subsample=0.8,
#         colsample_bytree=0.8,
...
# --- 2. HELPER FUNCTIONS ---
def load_data_from_supabase():
    """Fetch data from Supabase and preprocess dates."""
    url: str = os.environ.get("SUPABASE_URL")
    key: str = os.environ.get("SUPABASE_KEY")
    supabase: Client = create_client(url, key)
    response = supabase.table('agg_trans').select("*").order('date').execute()
...
def create_features(df, city='beirut'):
    """Enhanced feature engineering for Beirut-specific forecasting."""
    df = df[df[GROUPING_KEY] == city].copy()  # Filter for Beirut only
    df_features = df.copy()
    ...
    df_features['rolling_mean_3'] = df_features[TARGET_COL].rolling(3).mean()
    
    return df_features.dropna()

def plot_validation_results(train_df, validation_df, predictions, city_name):
    """Plot actual vs. predicted values."""
    plt.figure(figsize=(12, 6))
    plt.plot(train_df.index, train_df[TARGET_COL], label='Training Data', color='blue')
    plt.plot(validation_df.index, validation_df[TARGET_COL], label='Actual', color='green')
    plt.plot(validation_df.index, predictions, label='Predicted', color='red', linestyle='--')
    plt.title(f'Beirut Forecast: Actual vs. Predicted', fontsize=14)
    plt.legend()
    plt.grid(True)
    plt.show()
...
# --- 3. MAIN WORKFLOW ---
if __name__ == "__main__":
    print("--- Beirut-Specific Forecasting Model ---")
    
    # Load and preprocess data
    all_data = load_data_from_supabase()
    beirut_data = create_features(all_data, city='beirut')
    
    # Split into train/validation (2011-2015 vs. 2016-2017)
    split_date = '2016-01-01'
    train = beirut_data[beirut_data.index < split_date]
    test = beirut_data[beirut_data.index >= split_date]
    ...
    # Plot results
    plot_validation_results(train, test, predictions, 'Beirut')
    
    # Save model
    joblib.dump(model, MODEL_FILE_PATH)
    pd.Series(X_train.columns.tolist()).to_json(MODEL_COLS_PATH)
    print("--- Model saved ---")
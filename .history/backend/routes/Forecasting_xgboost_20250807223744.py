...
if __name__ == "__main__":
   ...
   # --- Step 7: Create the Unified Forecast Visualization (CORRECTED) ---
    ...
    if not city_test_df.empty:
        # Use the city_test_df's index to filter the predictions.
        # The predictions arrays (mean_preds, p10_preds, etc.) have the same order and length
        # as the full y_test. We need to select the right rows.
        
        # Get the integer indices of the city's rows within the larger test set
        city_indices = y_test.index.isin(city_test_df.index)

        city_mean_preds = mean_preds[city_indices]
        city_p10_preds = p10_preds[city_indices]
        city_p90_preds = p90_preds[city_indices]

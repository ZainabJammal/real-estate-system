...
def prepare_apartment_data(df):
    ...
    print("Step X: Engineering size categories...")

    # Define the bins and labels
    bins = [0, 80, 120, 180, 250, 400, float('inf')]
    labels = ['Studio/Small (40-80)', 'Standard (81-120)', 'Comfortable (121-180)', 'Large (181-250)', 'Very Large (251-400)', 'Penthouse (401+)']
    ...
    df['size_category'] = pd.cut(df['size_m2'], bins=bins, labels=labels, right=False)
    X = df.drop(columns=['id', 'price_$', 'size_m2', ...]) # Drop the original size_m2
    y = df['price_$']

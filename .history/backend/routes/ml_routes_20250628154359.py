...
# Mayssoun's bit
# Define the base directory for your backend
...
# Load all models and supporting artifacts at startup
def load_artifacts():
    ...
    try:
        ...
        # artifacts['model_office'] = joblib.load(os.path.join(PRICE_MODEL_DIR, 'model_office.joblib'))
        # artifacts['model_shop'] = joblib.load(os.path.join(PRICE_MODEL_DIR, 'model_shop.joblib'))
        ...
        print("-> Price estimation models loaded successfully.")
        ...
        print(f"FATAL ERROR: Could not load a model file. {e}")
        print("Please ensure all training scripts have been run successfully.")
        return None
    return artifacts
...
async def forecast_with_xgboost(city_name: str):
    """
    Generates a 5-year 'Best Guess' forecast.
    (This logic is mostly kept from your original code, just uses the loaded model)
    """
    if not ML_ARTIFACTS or not supabase:
        return jsonify({"error": "Forecasting service or database connection is not available."}), 503

    print(f"\nReceived forecast request for: '{city_name}'")
    city_name = city_name.lower() # Standardize

    try:
        # Fetch historical data
        response = supabase.table('agg_trans').select('date, transaction_value').ilike('city', city_name).order('date').execute()
        if not response.data:
        ...
        # Format response
        forecast_json = [{'date': date.strftime('%Y-%m-%d'), 'predicted_value': val} for date, val in zip(future_dates, predictions)]
        
        return jsonify({
            "city_forecasted": city_name,
            "monthly_forecast": forecast_json
        })

...
def create_and_train_pipeline(X_train, y_train, X_test, y_test, params, feature_config):
    """Create, train, and evaluate a model pipeline"""
    preprocessor = ColumnTransformer(
        transformers=[
            ('num', StandardScaler(), feature_config['numerical']),
            ('cat', OneHotEncoder(handle_unknown='ignore', sparse_output=False), feature_config['categorical'])
...
def main():
    ...
    MODEL_CONFIG = {
        ...
        'office': {
            ...
            'features': {
                'numerical': ['size_m2', 'latitude', 'longitude', 'bedrooms'],
                'categorical': ['province', 'district', 'location_cluster']
            }
        },
        'shop': {
        ...
        'house_villa': {
            ...
            'features': {
                'numerical': ['size_m2', 'bedrooms', 'bathrooms', 'luxury_score', 'distance_to_center'],
                'categorical': ['district', 'location_cluster']
            }
        },
        'chalet': {
            'params': {
            ...
            'features': {
                'numerical': ['size_m2', 'bedrooms', 'distance_to_city', 'seasonal_factor'],
    ...
    df = df[df['type'] != 'Land'].copy()
    print(f"Working with {len(df)} properties after filtering.")
    ...
    # City center coordinates (example - replace with your actual city center)
    CITY_CENTER = (40.7128, -74.0060)  # NYC as example
    
    # Distance features
    df['distance_to_center'] = df.apply(
        lambda x: haversine(x['latitude'], x['longitude'], *CITY_CENTER), axis=1)
    
    # Luxury score for houses/villas
    df['luxury_score'] = (df['bathrooms'] * 0.3 + 
                         df['bedrooms'] * 0.2 + 
                         np.log1p(df['size_m2']) * 0.5)
    
    # Seasonal factor for chalets (assuming winter months are more valuable)
    df['seasonal_factor'] = 1.0  # Default value
    # Add your actual seasonal calculation here based on date
    
    # --- TRAINING SPECIALIST MODELS ---
    specialist_types = ['apartment', 'office', 'shop', 'house_villa', 'chalet']
    
    for prop_type in specialist_types:
        print(f"\n--- Training Specialist Model for: {prop_type.upper()} ---")
        
        # Handle type name variations
        search_type = prop_type.replace('_', '/')
        df_slice = df[df['type'].str.lower() == search_type.lower()].copy()
        
        if len(df_slice) < 50:
            print(f"   -> Insufficient data ({len(df_slice)} samples), skipping specialist model.")
            continue
        ...
        config = MODEL_CONFIG[prop_type]
        pipeline, r2 = create_and_train_pipeline(X_train, y_train, X_test, y_test, 
                                               config['params'], config['features'])
        
        if r2 > 0.6:  # Slightly lowered threshold for chalet/house
            model_path = os.path.join('final_models', f'model_{prop_type}.joblib')
            joblib.dump(pipeline, model_path)
            ...
            print(f"   -> Model performance below threshold (R²={r2:.2f})")
    ...
    X_general = df.drop(columns=['price_$'])
    y_general = np.log1p(df['price_$'])
    X_train_gen, X_test_gen, y_train_gen, y_test_gen = train_test_split(
        X_general, y_general, test_size=0.2, random_state=42)
    
    general_pipeline, gen_r2 = create_and_train_pipeline(
        X_train_gen, y_train_gen, X_test_gen, y_test_gen,
        general_config['params'], general_config['features'])
    
    model_path = os.path.join('final_models', 'model_general_fallback.joblib')
    joblib.dump(general_pipeline, model_path)
    print(f"   -> Generalist Fallback Model (R²={gen_r2:.2f}) saved to '{model_path}'")
    
    # Save supporting models
    joblib.dump(kmeans, os.path.join('final_models', 'kmeans_model.joblib'))
    
    print("\n--- Workflow Complete ---")
    print("Models saved to 'final_models' directory")

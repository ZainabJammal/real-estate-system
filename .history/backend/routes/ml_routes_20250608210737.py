...
# Estimation of Current Property Price# --- Helper function to fetch unique, non-empty values for a column ---
async def fetch_distinct_values(supabase_client, table_name, column_name):
    ...
    """
    try:
        # The .not_.is_() method is the correct way to filter for non-null values
        res = await supabase_client.from_(table_name) \
                                .select(column_name) \
        ...
        # --- Start of the robust processing logic ---
        
        # Check if the column is intended to be numeric (e.g., bedrooms, bathrooms)
        is_numeric_column = column_name in ["bedrooms", "bathrooms"]
        
        processed_values = set()
        if is_numeric_column:
            for val in raw_values:
                # Try to convert each value to a float. Skip if it fails.
                # This handles integers, floats, and filters out strings like ''
                try:
                    num_val = float(val)
                    # We want integers for dropdowns, so we convert valid floats to ints
                    processed_values.add(int(num_val))
                except (ValueError, TypeError):
                    # Ignore values that cannot be converted to a number
                    continue
        else:
            # For non-numeric columns (district, type), just filter out empty strings
            for val in raw_values:
                if isinstance(val, str) and val.strip():
                    processed_values.add(val.strip())
                elif val is not None: # Keep non-string values if they exist
                     processed_values.add(val)
        
        return sorted(list(processed_values))
        # --- End of the robust processing logic ---

    except Exception as e:
        print(f"ERROR fetching distinct values for '{column_name}': {e}")
        # Add the traceback to see the exact line causing the error
        import traceback
        traceback.print_exc()
...
async def get_property_input_options():
    ...
    try:
        ...
        results = await asyncio.gather(
            ...
            # For min/max, we also use the corrected filter

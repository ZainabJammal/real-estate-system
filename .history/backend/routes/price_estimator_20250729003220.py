...
def setup_environment():
    ...
    # ### --- THIS IS THE FIX --- ###
    # Get the directory the current script is in (which is '.../backend/routes/')
    script_dir = os.path.dirname(os.path.abspath(__file__))
    
    # Go UP ONE LEVEL to get the backend directory
    backend_dir = os.path.dirname(script_dir)
    
    # Now, build the output path inside the 'backend' directory
    output_dir = os.path.join(backend_dir, 'final_model_output_v8_supabase')
...
# --- 2. DATA LOADING ---
def load_data_from_supabase():
    """Connects to Supabase and fetches all property data."""
    print("-> Connecting to Supabase to fetch training data...")
    ...
    try:
        # Fetch all columns and all rows from the 'properties' table
        response = supabase.table('properties').select("*").execute()
        
        # The data is in response.data
        if not response.data:
            raise ValueError("No data returned from Supabase. The table might be empty.")
        ...
        print(f"FATAL ERROR: Could not fetch data from Supabase. Error: {e}")
        return None

# --- 3. DATA PREPARATION & PLOTTING (Functions are unchanged) ---
def prepare_apartment_data(df):
    """Loads, cleans, and engineers features for the apartment dataset."""
    print("\n--- Step 3: Preparing Apartment Data ---")
...
def plot_correlation_heatmap(df, output_dir):
    ...
    correlation_matrix = df[corr_cols].corr()
    sns.heatmap(correlation_matrix, annot=True, cmap='coolwarm', fmt=".2f", linewidths=.5)
    plt.title('Correlation Matrix of Key Numerical Features'); plt.tight_layout()
    plot_path = os.path.join(output_dir, 'correlation_heatmap.png')
...
def plot_regression_results(y_true, y_pred, model_name, output_dir):
    ...
    plt.title(f"Actual vs. Predicted Prices ({model_name})"); plt.legend()
    plt.gca().ticklabel_format(style='plain', axis='both'); plt.grid(True); plt.tight_layout()
    plot_path = os.path.join(output_dir, f'regression_plot_{model_name.lower()}.png')
    plt.savefig(plot_path); plt.close()
    print(f"   -> Regression plot saved to '{plot_path}'")
...
def plot_feature_importance(pipeline, model_name, output_dir):
    ...
    plt.xlabel(f"{model_name} Feature Importance"); plt.ylabel("Feature")
    plt.title(f"Top 20 Feature Importances ({model_name})"); plt.tight_layout()
    plot_path = os.path.join(output_dir, f'feature_importance_{model_name.lower()}.png')
    plt.savefig(plot_path); plt.close()
    print(f"   -> Feature importance plot saved to '{plot_path}'")
    
# --- 4. MODEL TRAINING & EVALUATION WORKFLOW ---
def main():
    output_dir = setup_environment()
    
    # Step 1: Load data from Supabase instead of CSV
    df_raw = load_data_from_supabase()
    if df_raw is None or df_raw.empty:
        print("Halting execution due to data loading failure.")
        return

    # Step 2: Prepare data (this remains the same)
    df_featured = prepare_apartment_data(df_raw)
    
    # Step 3: Generate EDA plot
    plot_correlation_heatmap(df_featured, output_dir)
    ...
    print("\n--- Step 4: Splitting Data into Training (70%) and Testing (30%) Sets ---")
    X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=0.3, random_state=42)
    print(f"   -> Training set size: {len(X_train)} rows")
    print(f"   -> Testing set size:  {len(X_test)} rows")
    ...
    # ... (The rest of your main function for training/evaluating models remains exactly the same) ...
    # ... (It will now run on the data fetched from Supabase) ...

    # ==============================================================================
    # --- Part 1: Training and Evaluating the LightGBM Model ---
    # ==============================================================================
    print("\n" + "="*20 + " LightGBM Model Workflow " + "="*20)
    pipeline_lgbm = Pipeline(steps=[('preprocessor', preprocessor), ('regressor', lgb.LGBMRegressor(objective='regression_l1', metric='mae', random_state=42))])
    param_grid_lgbm = {'regressor__n_estimators': [1000, 1500], 'regressor__learning_rate': [0.01, 0.02], 'regressor__num_leaves': [31, 40], 'regressor__max_depth': [7, 9], 'regressor__l2_leaf_reg': [3, 5]}
    print("\n--- Running GridSearchCV for LightGBM (with 5-Fold CV) ---")
    grid_search_lgbm = GridSearchCV(pipeline_lgbm, param_grid_lgbm, cv=5, scoring='r2', n_jobs=-1, verbose=1)
    grid_search_lgbm.fit(X_train, y_train)
    best_lgbm_model = grid_search_lgbm.best_estimator_
    ...
    # ==============================================================================
    print("\n" + "="*20 + " XGBoost Model Workflow " + "="*20)
    pipeline_xgb = Pipeline(steps=[('preprocessor', preprocessor), ('regressor', xgb.XGBRegressor(objective='reg:squarederror', eval_metric='mae', random_state=42, n_jobs=-1))])
    param_grid_xgb = {'regressor__n_estimators': [1000, 1500], 'regressor__learning_rate': [0.01, 0.02], 'regressor__max_depth': [6, 8], 'regressor__subsample': [0.7, 0.8], 'regressor__colsample_bytree': [0.7, 0.8]}
    print("\n--- Running GridSearchCV for XGBoost (with 5-Fold CV) ---")
    grid_search_xgb = GridSearchCV(pipeline_xgb, param_grid_xgb, cv=5, scoring='r2', n_jobs=-1, verbose=1)
    grid_search_xgb.fit(X_train, y_train)
    best_xgb_model = grid_search_xgb.best_estimator_
    ...
    y_pred_log_lgbm = best_lgbm_model.predict(X_test)
    y_pred_actual_lgbm = np.expm1(y_pred_log_lgbm)
    final_results['LightGBM'] = {'Best CV R²': grid_search_lgbm.best_score_, 'Test R²': r2_score(y_test, y_pred_log_lgbm), 'Test MAE ($)': mean_absolute_error(y_test_actual, y_pred_actual_lgbm), 'Best Params': grid_search_lgbm.best_params_}
    plot_regression_results(y_test_actual, y_pred_actual_lgbm, "LightGBM", output_dir)
    plot_feature_importance(best_lgbm_model, "LightGBM", output_dir)
    joblib.dump(best_lgbm_model, os.path.join(output_dir, 'model_apartment_lgbm_tuned.joblib'))
    ...
    y_pred_log_xgb = best_xgb_model.predict(X_test)
    y_pred_actual_xgb = np.expm1(y_pred_log_xgb)
    final_results['XGBoost'] = {'Best CV R²': grid_search_xgb.best_score_, 'Test R²': r2_score(y_test, y_pred_log_xgb), 'Test MAE ($)': mean_absolute_error(y_test_actual, y_pred_actual_xgb), 'Best Params': grid_search_xgb.best_params_}
    plot_regression_results(y_test_actual, y_pred_actual_xgb, "XGBoost", output_dir)
    plot_feature_importance(best_xgb_model, "XGBoost", output_dir)
    joblib.dump(best_xgb_model, os.path.join(output_dir, 'model_apartment_xgb_tuned.joblib'))
    ...
    # --- FINAL SUMMARY ---
    print("\n" + "="*60)
    print("--- FINAL MODEL COMPARISON SUMMARY (FROM SUPABASE DATA) ---")
    for model_name, metrics in final_results.items():
        print(f"\n--- {model_name} ---")
        print(f"   -> Best Cross-Validation R²: {metrics['Best CV R²']:.4f}")
        print(f"   -> Final Test Set R²:          {metrics['Test R²']:.4f}")

...
async def forecast_with_xgboost(user_selection: str):
    """
    Generates a 5-year forecast for a given city selection.
    """
    # First, map the user input to the correct city name for the model
    ...
    try:
        # === The rest of the logic remains the same, using the mapped 'city_name' ===
        ...
        parts = response.data['date'].str.split('-', expand=True)
        response.data['date'] = pd.to_datetime('01-' + parts[1] + '-' + parts[0], format='%d-%b-%y')
        city_history_df = response.data.set_index('date')
        ...
        city_avg_value = city_history_df[TARGET_COL].mean()
        monthly_avg_map = city_history_df.groupby(city_history_df.index.month)[TARGET_COL].mean()

        print(f"-> Step 3: Creating future dataframe for {FORECAST_HORIZON_MONTHS} months...")
        last_historical_date = city_history_df.index.max()
        ...
        return jsonify({
            "city_forecasted": user_selection, # Return the original selection for user context
            "historical_data": historical_json,
            "monthly_forecast": monthly_forecast_df.to_dict(orient='records'),
            "yearly_forecast": yearly_forecast_df.reset_index().rename(columns={'date': 'year', 'predicted_value': 'total_value'}).to_dict(orient='records')
        })

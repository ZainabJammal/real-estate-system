...
if __name__ == "__main__":
    ...
    # Calculate overall MAPE using the full test set
    mape = mean_absolute_percentage_error(y_test, predictions) * 100
    print(f"\nOverall Validation MAPE on 2016 data: {mape:.2f}%\n")

    # --- NEW, FOOLPROOF PLOTTING LOGIC ---
    ...
    # Create a clean results DataFrame. The index is the date.
    results_df = pd.DataFrame({
        'actual': y_test,
        'predicted': predictions
    })

    # We need the region name for each row. We can get this from the original
    # `data_with_features` DataFrame before it was one-hot encoded.
    results_df = results_df.join(data_with_features[GROUPING_KEY])

    # Now, results_df looks like:
    # date        | actual | predicted | city
    # ---------------------------------------------
    # 2016-01-01  | 140.5  | 155.2     | beirut
    # 2016-01-01  | 30.2   | 35.1      | north
    # ...

    # Plot the results for a specific region
    region_to_plot = 'beirut'
    print(f"-> Generating validation plot for region: '{region_to_plot}'")

    # Filter the results for just the region we want to plot
    plot_df = results_df[results_df[GROUPING_KEY] == region_to_plot]

    # Filter the original training data for the plot background
    ...
    if not plot_df.empty:
        ...
        # Plot actual values for 2016
        plt.plot(plot_df.index, plot_df['actual'], label='Actual Values (2016)', color='green', marker='o')
        # Plot predicted values for 2016
        plt.plot(plot_df.index, plot_df['predicted'], label='Predicted Values (2016)', color='red', linestyle='--')

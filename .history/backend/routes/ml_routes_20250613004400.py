...
# Get the absolute path of the directory where the API script is located
...
def load_and_preprocess_data(filepath):
    """Loads and prepares the historical data from the CSV."""
...
@ml_routes.route("/forecast/<string:city_name>", methods=["GET"])
async def predict_forecast(city_name: str):
    print(f"\nReceived request for city: {city_name}")
    
    try:
        # --- Validate Input ---
        # Check if the requested city exists in our historical data
        ...
        # --- Load Model and Columns (on each request, fast operation) ---
        ...
        # This is the recursive loop from our script
        ...
        # Convert dataframes to a list of dictionaries for JSON compatibility
        ...
        # Tweak yearly format to be cleaner
        ...
        # Use traceback to get more detailed error info in your server logs
        import traceback
        traceback.print_exc()

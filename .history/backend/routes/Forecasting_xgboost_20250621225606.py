...
#                 Reads from Supabase, Validates, and Saves Model
# ==============================================================================
...
# Define column names
# NOTE: The Supabase table uses 'city' as the region column name
GROUPING_KEY = 'city' 
...
def load_data_from_supabase():
    ...
    response = supabase.table('regional_transactions').select("*").order('date').execute()
...
if __name__ == "__main__":
    ...
    # --- Validate and Plot Results for a key region ---
    region_to_plot = 'beirut' # Use a lowercase region name
    print(f"\n--- Validating and Plotting for Region: {region_to_plot} ---")
    
    history = train_set_raw[train_set_raw[GROUPING_KEY] == region_to_plot].copy()
    validation_actuals = validation_set_raw[validation_set_raw[GROUPING_KEY] == region_to_plot]
    ...
    if not validation_actuals.empty:
        ...
        plot_validation_results(
            train_set_raw[train_set_raw[GROUPING_KEY] == region_to_plot], 
            validation_actuals, 
            predictions, 
            region_to_plot
        )
    else:
        print(f"No validation data found for region '{region_to_plot}'. Skipping plot.")

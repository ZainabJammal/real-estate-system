...
@app.route("/api/estimate_property_price", methods=["POST"])
async def estimate_price():
    try:
        ...
        df_input = pd.DataFrame([{
            ...
            "type": data.get("Type", "Apartment"),  # optional
...
# === Optional: Serve Property Input Options ===
@app.route("/api/property_input_options")
async def get_property_input_options():
    # You can replace these with queries from Supabase or the same CSV file
    return jsonify({
        "districts": ["Baabda", "Achrafieh", "Tripoli", "Zahle"],
        "types": ["Apartment", "Villa", "Land"],
        "bedroom_options": [0, 1, 2, 3, 4, 5],
        "bathroom_options": [0, 1, 2, 3],
        "size_range": {"min": 30, "max": 1000}
    })

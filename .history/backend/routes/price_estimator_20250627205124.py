...
def get_preprocessor(feature_config):
    ...
    return ColumnTransformer(
        ...
        remainder='passthrough' # Use passthrough to avoid errors with unlisted columns
    )
...
def train_specialist_model(df_type, property_type, kmeans_model, config):
    ...
    # Save the model only if it meets our quality threshold
    if r2 > 0.68:
...
def train_generalist_model(df_general, kmeans_model):
    ...
    general_params = {
        'objective': 'regression_l1', 'random_state': 42, 'n_estimators': 1000,
...
def main():
    """Main function to run the entire data processing and training workflow."""
    print("Starting the Hybrid Model Training Process...")
    
    # --- MODEL CONFIGURATION (Your expert tuning + our geo features) ---
    MODEL_CONFIG = {
        'apartment': {
            'params': {'objective':'regression_l1', 'random_state':42, 'depth': 8, 'learning_rate': 0.02, 'l2_leaf_reg': 3, 'iterations': 2500, 'bootstrap_type': 'Bayesian'},
            'features': {'numerical': ['size_m2', 'bedrooms', 'bathrooms'], 'categorical': ['province', 'district']}
        },
        'office': {
            'params': {'objective':'regression_l1', 'random_state':42, 'depth': 6, 'learning_rate': 0.03, 'l2_leaf_reg': 5, 'iterations': 2000, 'one_hot_max_size': 15},
            'features': {'numerical': ['size_m2', 'bedrooms', 'bathrooms'], 'categorical': ['province', 'district']}
        },
        'shop': {
            'params': {'objective':'regression_l1', 'random_state':42, 'learning_rate': 0.03, 'l2_leaf_reg': 3, 'iterations': 2000},
            'features': {'numerical': ['size_m2'], 'categorical': ['province', 'district']}
        },
        # These will be handled by the generalist model, but we define them to loop through
        'chalet': {}, 'house/villa': {}, 'land': {} 
    }
    ...
    df.drop(columns=['city', 'created_at'], inplace=True) # City is redundant with district/geo

    print("\nCreating global K-Means model for location clustering...")
    kmeans = KMeans(n_clusters=20, random_state=42, n_init='auto') # Increased clusters
    kmeans.fit(df[['latitude', 'longitude']])
    joblib.dump(kmeans, os.path.join('specialist_models', 'kmeans_model.joblib'))
    
    specialist_types = ['apartment', 'office', 'shop']
    
    # Train the specialist models
    ...
    # Prepare data for the generalist model (all types EXCEPT land)
    df_general = df[df['type'] != 'Land'].copy()

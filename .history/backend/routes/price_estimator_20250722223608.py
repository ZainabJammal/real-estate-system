...
import lightgbm as lgb
import matplotlib.pyplot as plt
import seaborn as sns  # Import seaborn for the correlation heatmap
import joblib
import warnings
import os
...
def setup_environment():
    ...
    output_dir = os.path.join(os.getcwd(), 'final_model_output_v6_with_plots')
...
# --- 2. DATA PREPARATION (with custom filtering) ---
def prepare_apartment_data(df):
    """Loads, cleans, and engineers features with specific filtering rules."""
    print("\n--- Step 2: Preparing Apartment Data with Custom Filtering ---")
    ...
    # --- Custom Filtering ---
    print("\n--- Filtering Data ---")
    original_rows = len(df_apartments)
    df_apartments = df_apartments[df_apartments['bedrooms'].between(1, 4)]
    df_apartments = df_apartments[df_apartments['bathrooms'].between(1, 4)]
    df_apartments = df_apartments[df_apartments['size_m2'] <= 450]
    print(f"   -> Filtering complete. Rows remaining: {len(df_apartments)} (from {original_rows})")
    ...
    # --- Final Transformation ---
    df_apartments_clean['log_price'] = np.log1p(df_apartments_clean['price_$'])
    print(f"   -> Final prepared data has {len(df_apartments_clean)} rows.")
    
    return df_apartments_clean

# --- 3. PLOTTING and EVALUATION ---

# ### --- NEW PLOT 1: Correlation Heatmap --- ###
...
def evaluate_model(pipeline, X_test, y_test):
    """Evaluates the model and returns metrics and prediction values for plotting."""
    print("\n--- Evaluating Model on Unseen Test Data ---")
    predictions_log = pipeline.predict(X_test)
    
    # Convert log predictions and log test values back to actual dollar amounts
    predictions_actual = np.expm1(predictions_log)
    y_test_actual = np.expm1(y_test)
    
    r2 = r2_score(y_test, predictions_log) # R² is calculated on log values
    mae = mean_absolute_error(y_test_actual, predictions_actual)
    
    print(f"   -> Test Set R² Score: {r2:.4f}")
    print(f"   -> Test Set MAE (in $): ${mae:,.2f}")
    
    # Return actual values for the regression plot
    return r2, mae, y_test_actual, predictions_actual
...
def plot_feature_importance(pipeline, output_dir):
    ...
    plt.xlabel("LightGBM Feature Importance")
    plt.ylabel("Feature")
    plt.title("Top 20 Feature Importances (Filtered Model)")
    plt.gca().invert_yaxis()
    plt.tight_layout()
...
def main():
    ...
    # The evaluate_model function now returns the actual values needed for the regression plot
    test_r2, test_mae, y_test_actual, predictions_actual = evaluate_model(best_model_pipeline, X_test, y_test)
    ...
    model_path = os.path.join(output_dir, 'model_apartment_tuned_filtered.joblib')
    ...
    print("--- MODEL PERFORMANCE SUMMARY ---")
    summary = {
        'Model': 'Apartment Price Estimator (Filtered)',

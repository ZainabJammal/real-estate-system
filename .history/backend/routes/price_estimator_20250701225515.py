...
pd.options.mode.chained_assignment = None
BACKEND_DIR = os.path.dirname(os.path.abspath(__file__))
OUTPUT_DIR = os.path.join(BACKEND_DIR, 'final_focused_models')
DATA_PATH = os.path.join(BACKEND_DIR, 'properties.csv')
os.makedirs(OUTPUT_DIR, exist_ok=True)
...
def main():
    """Main function to run the FOCUSED model training workflow."""
    print("="*60)
    print("Starting FOCUSED Model Training Process (Apartment & Commercial)...")
    print(f"-> Models will be saved to: '{os.path.abspath(OUTPUT_DIR)}'")
    print("="*60)

    # --- Step 1: Data Loading & Scoping ---
    df = pd.read_csv(DATA_PATH)
    df.drop_duplicates(subset=['id'], inplace=True)
    df.rename(columns={'type': 'property_type'}, inplace=True)
    df['property_type'] = df['property_type'].str.lower().str.strip()
    
    # *** OUR NEW FOCUSED STRATEGY ***
    APARTMENT_TYPES = ['apartment']
    COMMERCIAL_TYPES = ['office', 'shop']
    
    # Keep only the types we are targeting for our core product
    ...
    # Outlier handling on the target variable
    # We apply it once here since the groups are relatively similar
    ...
    for group_name, types in model_groups.items():
        ...
        categorical_features = ['province', 'district'] # No location cluster yet
        ...
        pipeline = Pipeline(steps=[
            ('preprocessor', preprocessor),
            ('regressor', lgb.LGBMRegressor(objective='regression_l1', random_state=42, n_estimators=1000, learning_rate=0.03))
        ])
    ...
    # --- Final Report ---
    print("\n" + "="*60)
    print("--- FOCUSED MODEL PERFORMANCE SUMMARY ---")
    summary_df = pd.DataFrame.from_dict(results_summary, orient='index')
    summary_df['R2_Score'] = summary_df['R2_Score'].map('{:.4f}'.format)
    summary_df['RMSE_Dollars'] = summary_df['RMSE_Dollars'].map('${:,.2f}'.format)
    print(summary_df)

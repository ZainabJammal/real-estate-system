...
async def fetch_numerical_options(supabase, column_name, is_int=True):
    res = await supabase.from_(PROPERTIES_TABLE_NAME).select(column_name).neq(column_name, "is.null").execute()
    if res.data:
        values = [item[column_name] for item in res.data if item[column_name] is not None]
        if not values: return [] if is_int else {}
        if is_int:
            return sorted(list(set(int(float(v)) for v in values)))
        else:
...
async def get_avg_coords_for_district(supabase, district_name):
    """Fetches the average latitude and longitude for a given district to use as a fallback."""
...
async def get_property_input_options():
    supabase = await create_supabase()
    try:
        # Corrected column names to match the schema image: 'bedrooms', 'bathrooms'
        ...
        bedroom_options = await fetch_numerical_options(supabase, "bedrooms", is_int=True)
        bathroom_options = await fetch_numerical_options(supabase, "bathrooms", is_int=True)
        ...
        traceback.print_exc()
        return jsonify({"error": "Failed to fetch property input options"}), 500
...
async def estimate_property_price():
    ...
    try:
        ...
        # Feature Engineering: Get average location for the district
        ...
        # Prepare feature dictionary for the model
        feature_dict = {

...
from quart import Blueprint, request, jsonify
from dotenv import load_dotenv
from geopy.distance import geodesic  # Ensure this import is here
...
def init_price_estimator_routes(): ### --- MODIFIED: Removed unused model_dir argument --- ###
    ...
    try:
        ...
        ### --- MODIFIED: Point to the new optimized model directory and file --- ###
        model_dir = os.path.join(backend_dir, 'final_model_output_v8_optimized')
        model_path = os.path.join(model_dir, 'model_apartment_xgb_optimized.joblib')
        ...
        print("-> Successfully loaded: 'model_apartment_xgb_optimized.joblib'")
...
async def predict_apartment_price():
    ...
    try:
        ...
        ### --- MODIFIED: Update the list of required fields --- ###
        # These fields must now be sent from the frontend.
        required_fields = [
        ...
        ### --- MODIFIED: Perform On-the-Fly Feature Engineering --- ###
        # The model expects the *exact same features* it was trained on.
        print("[PROCESS] Performing on-the-fly feature engineering...")
        
        # 1. Temporal Features (Using current year/month as a proxy for a new listing)
        ...
        input_df['dist_to_cbd_km'] = geodesic(
            (data['latitude'], data['longitude']), beirut_cbd
        ).km
        
        # 3. Log Transformation of size
        input_df['log_size_m2'] = np.log1p(input_df['size_m2'])
        
        # 4. Interaction Feature
        # Use a try-except block for safe division
        try:
            input_df['bed_bath_ratio'] = input_df['bedrooms'] / input_df['bathrooms']
        except ZeroDivisionError:
            input_df['bed_bath_ratio'] = np.nan # Let the imputer in the pipeline handle this
        
        # Handle potential inf values if bathrooms is 0
        input_df['bed_bath_ratio'].replace([np.inf, -np.inf], np.nan, inplace=True)
        
        print(f"[PROCESS] Data prepared for pipeline:\n{input_df[['log_size_m2', 'bedrooms', 'bathrooms', 'dist_to_cbd_km', 'district', 'city']]}")
        ...
        mae_from_training = 105348 # From your previous result, for a realistic range
        ...
        response_data = {
            "status": "success",
            "estimated_price": round(prediction_dollars, -2), # Round to nearest 100
            "price_range_low": max(0, round(price_range_low, -2)), # Ensure it's not negative
            "price_range_high": round(price_range_high, -2),
        }

...
# Initialize the Supabase client (you would do this once when your app starts)
# supabase: Client = create_client(SUPABASE_URL, SUPABASE_KEY)
# print("-> Supabase client initialized.")

# --- FIX 1: Correctly define paths to your THREE models ---
# This assumes your script is in a folder like /backend/routes/ and models are in /backend/forecasting_models/
try:
    SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))
    BACKEND_DIR = os.path.dirname(os.path.dirname(SCRIPT_DIR)) # Navigate two levels up
except NameError:
    SCRIPT_DIR = os.getcwd() # Fallback for interactive environments
    BACKEND_DIR = SCRIPT_DIR

FORECAST_MODEL_DIR = os.path.join(BACKEND_DIR, '_models')
...
def map_user_selection_to_city(selection_string: str) -> str:
    """Maps user selection to the lowercase city name used in training."""
    # This logic is specific to your frontend, adjust if necessary
    if "Tripoli" in selection_string: return "tripoli"
    if "Baabda" in selection_string: return "baabda"
    if "Kesrouan" in selection_string: return "kesrouan"
    if "Bekaa" in selection_string: return "bekaa"
    return selection_string.lower()
...
def generate_future_forecasts(city_name: str, historical_data: pd.DataFrame) -> pd.DataFrame:
    """
    Generates a 5-year forecast iteratively. This is the new core logic.
    """
    future_predictions = []
    
    # Use the last 13 months of historical data as the starting point for creating lags
    # This ensures lag_12 can be calculated correctly.
    current_data = historical_data.tail(13).copy()
    ...
    for i in range(FORECAST_HORIZON_MONTHS):
        ...
        # A. Create features based on the CURRENT state of our data history
        features_with_lags = create_features(current_data)
        
        # B. Prepare the single row of features for the future date we want to predict
        ...
        X_future = future_row_aligned.iloc[-1:] # Select the last row, which has features for our `next_date`

        # C. Predict using all three models
        pred_mean = MEAN_MODEL.predict(X_future)[0]
        pred_p10 = P10_MODEL.predict(X_future)[0]
        pred_p90 = P90_MODEL.predict(X_future)[0]
        
        # D. Store the results for this month
        ...
        # We use the mean forecast as the new "truth" for calculating the next month's lags.
...
# --- 6. THE REFACTORED API ENDPOINT ---
# @ml_routes.route("/forecast/xgboost/<string:user_selection>", methods=["GET"])
async def forecast_with_xgboost(user_selection: str):
    """
    Fetches historical data and uses an iterative process to generate a robust 5-year forecast.
    """
    # This is a placeholder for your actual Flask route
    # To run this function standalone for testing:
    # import asyncio
    # asyncio.run(forecast_with_xgboost("Beirut"))
    ...
    try:
        ...
        # Mocking the Supabase call for standalone testing:
        # Replace this with your actual Supabase call
        from supabase import create_client, Client
        supabase: Client = create_client(SUPABASE_URL, SUPABASE_KEY)
        response = supabase.table('merged_trans').select('date, transaction_value, city').eq(GROUPING_KEY, city_name).order('date').execute()
        ...
        # --- FIX 3: Update JSON to include the full forecast range ---
        monthly_forecast_df = forecast_df.reset_index()
        yearly_forecast_df = forecast_df.resample('YE').sum() # Resample on the DataFrame's DatetimeIndex

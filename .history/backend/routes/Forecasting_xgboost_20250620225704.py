...
def create_features(df):
    ...
    # Example 1: A specific, one-month event (e.g., policy change)
    df_features['policy_change_spike_mar2016'] = ((df_features.index.year == 2016) & (df_features.index.month == 3)).astype(int)

    # Example 2: An ongoing crisis period
    # Create a boolean series for the date range
    is_during_crisis = (df_features.index >= '2019-10-01')
    df_features['is_economic_crisis_period'] = is_during_crisis.astype(int)
    
    # You can add as many of these as you identify
    # df_features['post_port_explosion'] = (df_features.index >= '2020-08-04').astype(int)

    # --- END: NEW EXTERNAL SHOCK FEATURES ---

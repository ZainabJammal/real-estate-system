...
def train_and_evaluate_model():
    """
    V13: Definitive workflow using manual preprocessing and Bayesian Optimization.
    Fixes the ValueError by providing a proper search range for n_estimators.
    """
    ...
    # Preprocess data BEFORE tuning
    X_train_processed = preprocessor.fit_transform(X_train)
    # No need to transform X_test yet, the final pipeline will do it.

    # --- 5. Bayesian Optimization with BayesSearchCV ---
    print("\nStep 5: Setting up Bayesian Optimization (BayesSearchCV)...")
    
    # Define the search space with a proper range for n_estimators
    search_spaces = {
        'learning_rate': Real(0.01, 0.1, 'log-uniform'),
        # *** FIX: Provide a real search range for the number of trees ***
    ...
    # --- 7. Train Final Model and Re-assemble Pipeline ---
    ...
    # Fit the entire pipeline on the original (unprocessed) training data

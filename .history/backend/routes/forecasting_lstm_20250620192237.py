...
def create_lstm_sequences(X_data, y_data, look_back):
    ...
    # to have a corresponding y value.
...
if __name__ == "__main__":
    ...
    validation_final = validation_final.reindex(columns=FEATURES, fill_value=0)
    
    print(f"-> Data prepared. {len(FEATURES)} features created. Training set shape: {train_final.shape}")
    
    # --- Step 2: Scale the Data ---
    # Neural networks are sensitive to feature scales, so we scale both features and the target.
    ...
    X_val_scaled = feature_scaler.transform(validation_final[FEATURES])
    y_val_unscaled = validation_set_raw.loc[validation_final.index][TARGET_COL] # Get original y-values for evaluation
    ...
    X_val_seq, y_val_seq = create_lstm_sequences(X_val_scaled, y_val_unscaled.values, LOOK_BACK) # y_val_seq is unscaled for direct comparison
    ...
    # Align true values and predictions. y_val_seq contains the true values.
    ...
    # To create validation sequences, we need history from the training set.
    # So, we select all data for the city, both train and validation.
    all_city_data_raw = data_with_features[data_with_features['city'] == city_to_plot].copy()

    # One-hot encode and align features, same as the main dataset.
    city_features_encoded = pd.get_dummies(all_city_data_raw, columns=['city'], prefix='city')
    city_features_aligned = city_features_encoded.reindex(columns=FEATURES, fill_value=0).dropna()

    # # Scale the features for this city's entire history using the globally fitted scaler.
    # city_features_scaled = feature_scaler.transform(city_features_aligned)
    # # The target values are the unscaled originals.
    # city_target_unscaled = city_features_aligned[TARGET_COL]
    # Correct order:
    city_full_history_df = pd.get_dummies(all_city_data_raw, columns=['city'], prefix='city')
    city_features_aligned = city_full_history_df.reindex(columns=FEATURES, fill_value=0) # Re-indexing features
    city_target_unscaled = city_features_aligned[TARGET_COL] # Separating target after re-indexing

    # Now, create sequences from the city's *entire* history.
    # This correctly creates sequences that cross the train/validation boundary.
    city_X_seq, city_y_true = create_lstm_sequences(city_features_scaled, city_target_unscaled, LOOK_BACK)

    # The validation set for Beirut has 12 months.
    # The sequences we want to predict on are the *last 12* created from its history.
    ...
    if len(city_X_seq) < num_validation_points:
        raise ValueError(f"Not enough historical data for {city_to_plot} to generate validation predictions.")

    sequences_to_predict = city_X_seq[-num_validation_points:]
    true_values_for_plot = city_y_true[-num_validation_points:]

    # Predict on these specific validation sequences.
    ...
    # Prepare the raw training data for the plot background
    train_city_df = train_set_raw[train_set_raw['city'] == city_to_plot]

    # The validation_df for plotting needs to align with the predictions.
    # Since our predictions are for the full 12 months, we can use the full validation_city_df.
    plot_validation_results(train_city_df, validation_city_df, city_plot_predictions_unscaled)    # --- Step 5: Plot Validation Results for One City ---
    city_to_plot = 'Beirut'
    print(f"\n-> Generating validation plot for a single city: {city_to_plot}")
    ...
    # Drop rows with NaNs created by lag features. This aligns with the main training process.
    city_full_history_df.dropna(inplace=True)
    
    # *** THE FIX IS HERE ***
    # 1. Separate the target variable BEFORE re-indexing the features.
    #    The target exists in city_full_history_df.
    city_target_unscaled = city_full_history_df[TARGET_COL]
    
    # 2. Now, prepare the feature set by re-indexing to match the model's training columns.
    #    This DataFrame will NOT contain the target column, which is correct for X (feature) data.
    city_features_aligned = city_full_history_df.reindex(columns=FEATURES, fill_value=0)

    # Scale the aligned features using the globally fitted scaler.
    city_features_scaled = feature_scaler.transform(city_features_aligned)
    ...
    all_data_final = all_data_final.reindex(columns=FEATURES, fill_value=0) # Align columns just in case

    # Use the same scalers fitted on the original training data
    X_all_scaled = feature_scaler.transform(all_data_final[FEATURES])
    y_all_scaled = target_scaler.transform(all_data_final[[TARGET_COL]])
    ...
    production_model.fit(X_all_seq, y_all_seq, epochs=50, batch_size=32, verbose=1)

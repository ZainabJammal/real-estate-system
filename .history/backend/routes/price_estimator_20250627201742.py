...
MODEL_CONFIG = {
    ...
    'land': {
        'params': {
            'depth': 10,  # Increased depth for complex land valuation
            'learning_rate': 0.005,  # Lower learning rate for difficult problem
            'l2_leaf_reg': 15,  # Strong regularization for small dataset
            'iterations': 3000,  # More iterations needed
            'grow_policy': 'Depthwise',  # Better for small datasets
            'bootstrap_type': 'Bernoulli',  # Works well with strong regularization
            'random_strength': 2,  # More randomness for small dataset
            'border_count': 64,  # Lower for small dataset
            'early_stopping_rounds': 100,
            'min_data_in_leaf': 10  # Important for small dataset
        },
        'features': {
            'numerical': ['log_size_m2'],
            'categorical': ['province', 'district', 'city']
    ...
    'house/villa': {
        'params': {
            'depth': 7,  # Good middle ground
            'learning_rate': 0.015,  # Slightly reduced
            'l2_leaf_reg': 5,  # Moderate regularization
            'iterations': 2500,
            'grow_policy': 'Lossguide',  # Good for small-medium datasets
            'bootstrap_type': 'MVS',  # Modern alternative
            'random_strength': 1.5,
            'border_count': 128,
            'early_stopping_rounds': 75,
            'max_leaves': 64  # Helps control complexity
        },
        'features': {
    ...
    'shop': {
        'params': {
            'depth': 6,
            'learning_rate': 0.04,  # Slightly higher for shops
            'l2_leaf_reg': 5,  # Reduced from 7
            'iterations': 1800,
    ...
    'chalet': {
        'params': {
            'depth': 5,  # Reduced depth for very small dataset
            'learning_rate': 0.01,  # Lower learning rate
            'l2_leaf_reg': 10,  # Strong regularization
            'iterations': 2000,
            'grow_policy': 'Depthwise',
            'bootstrap_type': 'Bernoulli',
            'random_strength': 3,  # More randomness for small dataset
            'border_count': 64,
            'early_stopping_rounds': 100,
            'min_data_in_leaf': 5  # Important for small dataset
        },
        'features': {
            'numerical': ['log_size_m2', 'bedrooms', 'bathrooms'],
            'categorical': ['province', 'district', 'city']

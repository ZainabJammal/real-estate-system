...
# 1. CONFIGURATION AND PATH SETUP
# Ensure you are running this from a directory where __file__ is defined, or replace with an absolute path.
try:
    SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))
except NameError:
...
def load_and_preprocess_data(filepath):
    """
    Loads data, normalizes the 'city' column by splitting and exploding,
    and performs date-time preprocessing.
    """
    print("-> Loading and preprocessing data...")
    if not os.path.exists(filepath):
        raise FileNotFoundError(f"The specified CSV file was not found: {filepath}")
        
    df = pd.read_csv(filepath)

    # --- START: THE CORE FIX ---
    print("-> Normalizing 'city' column: splitting, exploding, and cleaning...")
    # 1. Split the comma-separated city strings into lists of strings
    df['city'] = df['city'].str.split(',')
    
    # 2. Explode the DataFrame to create a new row for each city in the list
    df = df.explode('city')
    
    # 3. Clean up any leading/trailing whitespace from the new city names
    df['city'] = df['city'].str.strip()
    # --- END: THE CORE FIX ---

    # Continue with the original date processing
    parts = df['date'].str.split('-', expand=True)
    df['date_str'] = '01-' + parts[1] + '-' + parts[0]
    df['date'] = pd.to_datetime(df['date_str'], format='%d-%b-%y')
    
    # Group by the now-normalized city and date to ensure one entry per city per month.
    # This handles cases where a city might have appeared twice in one original row.
    # We take the mean transaction value as a safe aggregation strategy.
    # We also need to re-introduce the 'id' if we want to drop it later, or just handle it here.
    df = df.groupby(['date', 'city']).agg({TARGET_COL: 'mean'}).reset_index()

    df = df.set_index('date')
    df.sort_index(inplace=True)
    
    # The 'id' and 'date_str' columns were effectively removed by the groupby, so no need to drop them.
    return df
...
def create_features(df):
    ...
    # These will now be calculated correctly for each INDIVIDUAL city
    df_features['lag_1'] = df_features.groupby('city')[TARGET_COL].shift(1)
    df_features['lag_3'] = df_features.groupby('city')[TARGET_COL].shift(3)
    df_features['lag_12'] = df_features.groupby('city')[TARGET_COL].shift(12)
    df_features['rolling_mean_3'] = df_features.groupby('city')[TARGET_COL].shift(1).rolling(window=3).mean()
    df_features['rolling_std_3'] = df_features.groupby('city')[TARGET_COL].shift(1).rolling(window=3).std()
...
def plot_validation_for_city_group(train_df_raw, validation_df_raw, model, feature_list, cities_to_plot, group_name):
    """
    Generates a validation plot for a group of cities by aggregating their values.
    """
    print(f"\n-> Generating aggregated validation plot for group: {group_name}")

    if not isinstance(cities_to_plot, list):
        cities_to_plot = [cities_to_plot]

    # --- 1. Aggregate the historical and validation data ---
    # Filter for the cities in the group and then sum their values by date
    train_group_df = train_df_raw[train_df_raw['city'].isin(cities_to_plot)].groupby(level=0)[TARGET_COL].sum().to_frame()
    validation_group_df = validation_df_raw[validation_df_raw['city'].isin(cities_to_plot)].groupby(level=0)[TARGET_COL].sum().to_frame()

    if validation_group_df.empty:
        print(f"Warning: No validation data found for any cities in the group '{group_name}'. Skipping plot.")
        return

    # --- 2. Prepare feature set for the group to make predictions ---
    validation_cities_features = validation_df_raw[validation_df_raw['city'].isin(cities_to_plot)]
    validation_cities_dummies = pd.get_dummies(validation_cities_features, columns=['city'], prefix='city')
    X_validation_cities = validation_cities_dummies.reindex(columns=feature_list, fill_value=0)
    
    # --- 3. Predict on the disaggregated data ---
    group_predictions_raw = model.predict(X_validation_cities)
    
    # --- 4. Aggregate the predictions ---
    # Create a temporary DataFrame to sum predictions by date
    predictions_df = pd.DataFrame({
        'predictions': group_predictions_raw,
        'date': X_validation_cities.index
    })
    aggregated_predictions = predictions_df.groupby('date')['predictions'].sum()

    # --- 5. Plot the aggregated results ---
    plt.figure(figsize=(15, 7))
    plt.plot(train_group_df.index, train_group_df[TARGET_COL], label=f'Training Data ({group_name})', color='blue')
    plt.plot(validation_group_df.index, validation_group_df[TARGET_COL], label=f'Actual Values ({group_name})', color='green', marker='o', linestyle='-')
    plt.plot(aggregated_predictions.index, aggregated_predictions.values, label=f'Predicted Values ({group_name})', color='red', linestyle='--')
    plt.title(f'Model Validation (Aggregated): {group_name}', fontsize=16)
    plt.xlabel('Date')
    plt.ylabel('Total Transaction Value')
    plt.legend()
    plt.grid(True)
    print("\n-> Displaying aggregated validation plot. Close the plot window to continue the script.")
    plt.show()
...
if __name__ == "__main__":
    ...
    # One-hot encode the now-atomic city names
    train_final_for_tuning = pd.get_dummies(train_set_raw, columns=['city'], prefix='city').dropna()
    FEATURES = [col for col in train_final_for_tuning.columns if col != TARGET_COL]
    X_train_tune, y_train_tune = train_final_for_tuning[FEATURES], train_final_for_tuning[TARGET_COL]
    ...
    # --- Step 4: Plot the Validation Results for ONE City ---
        # --- Step 4: Plot Validation Results ---

    # OPTION A: Plot a single city (the simple way)
    city_to_plot_single = 'Beirut'
    # Use the original plotting function
    # (Code for this is already in the full script I provided earlier)
    
    # OPTION B: Plot an aggregated group of cities (the advanced way)
    cities_to_aggregate = ['Baabda', 'Aley', 'Chouf']
    region_name = 'Baabda/Aley/Chouf Region'
    plot_validation_for_city_group(
        train_set_raw, 
        validation_set_raw, 
        optimized_model, 
        FEATURES, 
    ...
    # You can plot another group too!
    cities_to_aggregate_2 = ['Tripoli', 'Akkar']
    region_name_2 = 'Tripoli/Akkar Region'
    plot_validation_for_city_group(
        train_set_raw, 
        validation_set_raw, 
        optimized_model, 
        FEATURES, 
        cities_to_aggregate_2, 
        region_name_2
    )
    print(f"\n-> Generating validation plot for a single city: {city_to_plot}")
    ...
    if validation_city_df.empty:
        print(f"Warning: No validation data found for '{city_to_plot}'. Skipping plot.")
    else:
        validation_city_features = pd.get_dummies(validation_city_df, columns=['city'], prefix='city')
        X_validation_city = validation_city_features.reindex(columns=FEATURES, fill_value=0)
    ...
    # --- Step 5: Re-train Final Production Model on ALL Data ---
    print("\n-> Re-training final model on ALL data (2011-2016)...")
    all_data_with_dummies = pd.get_dummies(data_with_features, columns=['city'], prefix='city').dropna()
    
    # Ensure all columns from original tuning are present
    X_all = all_data_with_dummies.reindex(columns=FEATURES, fill_value=0)
    y_all = all_data_with_dummies[TARGET_COL]
    ...
    print(f"-> Saving final, optimized model to '{MODEL_FILE_PATH}'")
    joblib.dump(production_model, MODEL_FILE_PATH)
    # Save the list of feature columns (including the new dummified city columns)

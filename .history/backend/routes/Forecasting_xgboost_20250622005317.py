...
if __name__ == "__main__":
    ...
    # Filter the predictions for just that region
    region_test_indices = y_test[X_test[f'{GROUPING_KEY}_{region_to_plot}'] == 1].index
    region_predictions = pd.Series(predictions, index=y_test.index).loc[region_test_indices]
    
    if not plot_test_df.empty:
        # We need a new plotting function for this non-recursive forecast
        plt.figure(figsize=(15, 7))
        ...
        print(f"No validation data to plot for {region_to_plot}.")

...
# --- 1. SETUP & CONFIGURATION (Unchanged, but correct) ---
...
ef load_data_from_supabase():
    """Connects to Supabase and fetches ALL property data, ensuring correct types."""
    ...
    # --- FIX #1: Added .limit(3000) to fetch all rows ---
    print("   -> Fetching up to 3000 rows from the 'properties' table...")
    response = supabase.table('properties').select("*").limit(3000).order('id').execute()
    
    df = pd.DataFrame(response.data)
    print(f"-> Successfully fetched {len(df)} rows from Supabase.")

    # --- FIX #2: Explicitly convert columns to numeric types (Your existing code, which is correct) ---
    print("   -> Converting data types to numeric...")
    numeric_cols = ['price_$', 'size_m2', 'bedrooms', 'bathrooms', 'latitude', 'longitude']
...
# --- 2. DATA PREPARATION (REVISED FOR BEST PRACTICES) ---
def prepare_apartment_data(df):
    """Cleans and engineers features for the apartment dataset."""
    print("\n--- Step 2: Preparing Apartment Data ---")
    print(f"Initial rows: {len(df)}")
    df_apartments = df[df['property_type'] == 'apartment'].copy()
    print(f"Rows after filtering for apartments: {len(df_apartments)}")
    

    df_apartments = df[df['property_type'] == 'apartment'].copy()

    # FIXED: Replaced `inplace=True` with assignment to avoid warnings
    df_apartments['bedrooms'] = df_apartments['bedrooms'].replace(0, np.nan)
    df_apartments['bathrooms'] = df_apartments['bathrooms'].replace(0, np.nan)
    print(f"Rows before dropna: {len(df_apartments)}")
    df_apartments.dropna(subset=['price_$', 'size_m2', 'bedrooms', 'bathrooms'], inplace=True)
    print(f"Rows AFTER dropna: {len(df_apartments)}")
    
    # This will now correctly drop rows where the numeric conversion failed (became NaN)
    df_apartments.dropna(subset=['price_$', 'size_m2', 'bedrooms', 'bathrooms'], inplace=True)
    
    df_apartments = df_apartments[df_apartments['size_m2'] > 0]
    
    # ... (The rest of your feature engineering is perfect) ...
...
# ... (The rest of your functions: build_and_evaluate_model, plot_feature_importance are perfect) ...
def build_and_evaluate_model(df_featured, config, output_dir):
    # ... (Your function is perfect, no changes needed) ...
    ...
    preprocessor = ColumnTransformer(transformers=[('num', Pipeline(steps=[('imputer', SimpleImputer(strategy='median')),('scaler', StandardScaler())]), UPDATED_NUMERICAL_FEATURES),('cat', Pipeline(steps=[('imputer', SimpleImputer(strategy='most_frequent')),('onehot', OneHotEncoder(handle_unknown='ignore', sparse_output=False))]), CATEGORICAL_FEATURES)],remainder='drop')
    pipeline = Pipeline(steps=[('preprocessor', preprocessor),('regressor', lgb.LGBMRegressor(**config['params']))])
...
def plot_feature_importance(pipeline, output_dir):
    ...
    plt.figure(figsize=(10, 8)); plt.barh(importances['feature'], importances['importance']); plt.xlabel("LightGBM Feature Importance"); plt.ylabel("Feature"); plt.title("Top 20 Feature Importances"); plt.gca().invert_yaxis(); plt.tight_layout()
    plot_path = os.path.join(output_dir, 'feature_importance_advanced.png'); plt.savefig(plot_path); print(f"   -> Feature importance plot saved to '{plot_path}'"); plt.close()

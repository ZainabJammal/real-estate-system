...
# 2. Define a SINGLE, simple test route.
#    NO database logic, NO pandas. Just a message.
@market_simulator_bp.route("/api/market-simulator", methods=["GET"])
async def get_market_scenarios():
    print("--- SUCCESS: The /v1/api/market-simulator route was reached! ---")
    return jsonify({"message": "Success! The API route is working."})
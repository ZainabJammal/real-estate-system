...
#     # --- 2. THE FIX: Create the directory FIRST ---
#     # The 'exist_ok=True' prevents an error if the folder already exists.
#     os.makedirs(output_dir, exist_ok=True)
#     print(f"-> Ensured output directory exists: '{output_dir}'")

#     # --- 3. Now, save all the files into the guaranteed-to-exist directory ---
    
#     # Save the generalist model
#     model_path = os.path.join(output_dir, 'model_general_fallback.joblib')
#     joblib.dump(general_pipeline, model_path)
#     # Corrected print statement to use the right variable
#     print(f"   -> Generalist Fallback Model saved to '{model_path}'")

#     # Save the feature importance plot
#     plot_feature_importance(general_pipeline, "general_fallback", output_dir) # Pass the correct dir to the function
...
def plot_feature_importance(pipeline, prop_type, output_dir): # Added output_dir argument
    ...
    # Use the passed-in output_dir to save the file
...
def create_and_train_pipeline(X_train, y_train, X_test, y_test, params, feature_config):
    """Creates, trains, and evaluates a model pipeline."""
...
def main():
    ...
    # --- *** THE FIX: Define correct paths at the very top of main() *** ---
    # Assumes this training script is in backend/routes/ or similar
    SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))
    BACKEND_DIR = os.path.dirname(SCRIPT_DIR)
    OUTPUT_DIR = os.path.join(BACKEND_DIR, 'final_models')
    
    # Create the output directory right away to ensure it exists
    os.makedirs(OUTPUT_DIR, exist_ok=True)
    ...
    # --- TRAINING SPECIALIST MODELS ---
    specialist_types = ['apartment', 'office', 'shop']
    df_for_generalist = df[~df['type'].isin(specialist_types)].copy()
    ...
    for prop_type in specialist_types:
        ...
        if len(df_slice) < 50:
            print(f"   -> Insufficient data ({len(df_slice)} rows). Adding to generalist pool.")
            df_for_generalist = pd.concat([df_for_generalist, df_slice])
            continue
        ...
        if r2 > 0.65:
            ...
            joblib.dump(pipeline, model_path)
            print(f"   -> High-quality model saved to '{model_path}'")
            plot_feature_importance(pipeline, prop_type, OUTPUT_DIR) # Pass the correct dir
        else:
            print(f"   -> Model performance below threshold. Adding to generalist pool.")
            df_for_generalist = pd.concat([df_for_generalist, df_slice])

    # --- TRAINING GENERALIST FALLBACK MODEL ---
    print("\n--- Training Generalist Fallback Model ---")
    print(f"The generalist model will be trained on {len(df_for_generalist)} samples.")
    ...
    X_general = df_for_generalist.drop(columns=['price_$'])
    y_general = np.log1p(df_for_generalist['price_$'])
    X_train_gen, X_test_gen, y_train_gen, y_test_gen = train_test_split(X_general, y_general, test_size=0.2, random_state=42)

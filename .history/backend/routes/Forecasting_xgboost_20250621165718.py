...
# e.g., 'C:/.../backend/routes'
SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))

# Build the absolute path to the project's root directory
# os.path.dirname(SCRIPT_DIR) goes from '.../routes' to '.../backend'
# os.path.dirname(os.path.dirname(SCRIPT_DIR)) goes from '.../backend' to '.../real-estate-system'
PROJECT_ROOT_DIR = os.path.dirname(os.path.dirname(SCRIPT_DIR))

# Define the absolute path to the 'data' folder
DATA_DIR = os.path.join(PROJECT_ROOT_DIR, 'data')

# Define the absolute path to our clean input file
CSV_FILE_PATH = os.path.join(DATA_DIR, 'backend\data\regional_transactions.csv') 

# Define output paths for the model artifacts. These will be saved
# in the same directory as the script for simplicity.
MODEL_FILE_PATH = os.path.join(SCRIPT_DIR, 'forecast_model.joblib')
MODEL_COLS_PATH = os.path.join(SCRIPT_DIR, 'model_columns.json')
...
def load_and_preprocess_data(filepath):
    ...
    df = pd.read_csv(filepath)
    # The date format from our prepare_data script is standard YYYY-MM-DD

...
if __name__ == "__main__":
    ...
    print("-> Training the XGBoost model on pre-2016 data...")
        # A robust set of hyperparameters for a small dataset
    xgb_params = {
        'objective': 'reg:squarederror',
        'random_state': 42,

        # Controls complexity - keep these low to prevent overfitting
        'max_depth': 4,          # A shallow tree. 3-5 is a good range.
        'learning_rate': 0.05,   # A small learning rate requires more trees but is more robust.
        'n_estimators': 500,     # A moderate number of trees.

        # Regularization - These are your most important weapons against overfitting
        'subsample': 0.7,        # Use 70% of data for each tree.
        'colsample_bytree': 0.7, # Use 70% of features for each tree.
        'gamma': 0.1,            # Requires a larger loss reduction to make a split.
...
# --- End of new validation/plotting block ---
    ...
    production_model = xgb.XGBRegressor(
        ...
        # random_state=42
        objective: 'reg:squarederror',  
        n_estimators: 150,          # Small dataset → fewer trees
        max_depth: 3,               # Shallow to avoid overfitting
        learning_rate: 0.05,
        subsample: 0.8,
        colsample_bytree: 0.8,
        gamma: 0.1,                 # Regularization
        min_child_weight: 2,         # Controls splits
        reg_alpha: 0.1,             # L1 regularization

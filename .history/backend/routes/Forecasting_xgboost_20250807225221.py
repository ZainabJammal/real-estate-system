...
def create_features(df):
    """
    Creates time series features from the datetime index. This now includes
    lags, seasonal features, and rolling window features.
    """
    df_features = df.copy()
    ...
    # We calculate statistics over past periods to capture trends and volatility.
    # IMPORTANT: We .shift(1) to prevent data leakage from the current month.
    window_sizes = [3, 6, 12]
    for window in window_sizes:
        # Rolling Mean (Trend)
        df_features[f'rolling_mean_{window}'] = df_features.groupby(GROUPING_KEY)[TARGET_COL].shift(1).rolling(window=window).mean()
        
        # Rolling Standard Deviation (Volatility)
        df_features[f'rolling_std_{window}'] = df_features.groupby(GROUPING_KEY)[TARGET_COL].shift(1).rolling(window=window).std()
    ...
    # Fill initial NaNs created by lags and rolling windows.
    # It's important this comes AFTER all feature creation.
    df_features.fillna(0, inplace=True)
            
    return df_featuresdef create_features(df):
    """
    Creates time series features from the datetime index. This now includes
    lags, seasonal features, and rolling window features.
    """
...
if __name__ == "__main__":
  ...
  # --- Step 7: Create the Unified Forecast Visualization (FINAL CORRECTED VERSION 2) ---
    print("\n--- Generating the Unified Forecast Plot ---")

    city_to_plot = 'beirut'
    print(f"-> Generating validation plot for a single region: '{city_to_plot}'")

    # Isolate the data for the specific city from the ONE-HOT ENCODED test set
    city_test_df = test_df[test_df[f'city_{city_to_plot}'] == 1]

    # Check if there is anything to plot
    if not city_test_df.empty:
        # Get the boolean mask indicating which rows in the full test set belong to our city.
        # This DIRECTLY returns a NumPy boolean array.
        city_indices_mask = y_test.index.isin(city_test_df.index)

        # **THE FIX IS HERE**: We use the NumPy mask directly, without .values
        city_mean_preds = mean_preds[city_indices_mask]
        city_p10_preds = p10_preds[city_indices_mask]
        city_p90_preds = p90_preds[city_indices_mask]
        
        # Isolate the original (non-encoded) training data for the same city for a continuous plot
        train_city_df = data_with_features[
            (data_with_features.index < split_date) & 
            (data_with_features[GROUPING_KEY] == city_to_plot)
        ]
        ...
        plt.plot(city_test_df.index, city_test_df[TARGET_COL], label='Actual Values', color='green', marker='o', linestyle='-')
        
        # Plot the model forecasts
        plt.plot(city_test_df.index, city_mean_preds, label='Mean Forecast', color='red', linestyle='--')
        plt.plot(city_test_df.index, city_p10_preds, label='P10 Forecast', color='#663399', linestyle=':')
        plt.plot(city_test_df.index, city_p90_preds, label='P90 Forecast', color='#FF8C00', linestyle=':')
        
        # Add the confidence interval shading
        plt.fill_between(city_test_df.index, city_p10_preds, city_p90_preds, color='grey', alpha=0.2, label='80% Confidence Interval')
        ...
        print(f"-> WARNING: No validation data found to plot for region '{city_to_plot}'.")


     # --- Step 7: Save All Production Artifacts (CRITICAL) ---
    ...
    mean_model_path = os.path.join(OUTPUT_DIR, 'mean_forecast_model.joblib')
    p90_model_path = os.path.join(OUTPUT_DIR, 'p90_forecast_model.joblib')
    p10_model_path = os.path.join(OUTPUT_DIR, 'p10_forecast_model.joblib')
    features_path = os.path.join(OUTPUT_DIR, 'model_features.json')

    # Save the models
    joblib.dump(mean_model, mean_model_path)
    joblib.dump(p90_model, p90_model_path)
    joblib.dump(p10_model, p10_model_path)
    
    # Save the feature list
    pd.Series(FEATURES).to_json(features_path, indent=4)
    
    print(f"-> Mean model saved to: {mean_model_path}")
    print(f"-> P90 model saved to: {p90_model_path}")
    print(f"-> P10 model saved to: {p10_model_path}")

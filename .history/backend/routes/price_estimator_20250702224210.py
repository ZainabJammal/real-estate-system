...
def main():
    ...
    df = pd.read_csv(data_path); df.drop_duplicates(subset=['id'], inplace=True); df.rename(columns={'type': 'property_type'}, inplace=True); df['property_type'] = df['property_type'].str.lower().str.strip()
    ...
    model_path = os.path.join(output_dir, 'model_apartment_advanced.joblib'); joblib.dump(final_pipeline, model_path); print(f"\n--- Step 6: Final model saved to '{model_path}' ---")
    print("\n" + "="*60); print("--- MODEL PERFORMANCE SUMMARY ---")

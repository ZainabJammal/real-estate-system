...
# ==============================================================================
#           MAYSNOUN'S BIT - FINAL "BEST GUESS" API (NON-RECURSIVE)
# ==============================================================================
import pandas as pd
import numpy as np
import joblib
...
supabase: Client = create_client(SUPABASE_URL, SUPABASE_KEY)
print("-> Supabase client initialized for 'Best Guess' model.")
...
# --- 2. HELPER FUNCTIONS (Must mirror the 'Best Guess' training script) ---

def map_user_selection_to_city(selection_string: str) -> str:
    """Maps user selection to the specific lowercase city name from the database."""
    ...
    if "baabda" in lowered_selection and "aley" in lowered_selection:
        return "baadbad" # Make sure this matches your DB: 'baadbad' or 'baabda'?
...
def create_features(df):
    """
    Creates ONLY features that can be known in advance.
    NO LAGS OR ROLLING FEATURES. Must match the training script.
    """
    df_features = df.copy()
    df_features['year'] = df_features.index.year
    df_features['month_sin'] = np.sin(2 * np.pi * df_features.index.month / 12.0)
...
async def forecast_with_xgboost(user_selection: str):
    ...
    try:
        ...
        # Step 3: Create features for the future DataFrame
        ...
        if response.data:
            ...
            # Use your original robust date parsing logic for historical display
            parts = hist_df['date'].astype(str).str.split('-', expand=True)

...
if __name__ == "__main__":
   ...
   # --- Step 1: Load and Prepare Data ---
    ...
    if not X_validation_city.empty:
        city_plot_predictions = production_model_model.predict(X_validation_city)
    ...
    # We will use the same robust hyperparameters we defined earlier.
    # No need to get clever with best_iteration.
    # --- Step 6: Re-train Final Model on ALL Data ---
    print("\n-> Validation complete. Re-training final model on ALL data for production...")
    X_all, y_all = data_final[FEATURES], data_final[TARGET_COL]
    ...
    xgb.plot_importance(vamodel, height=0.8)

...
async def fetch_distinct_values(supabase_client, table_name, column_name):
    ...
    try:
        ...
        if res.data:
            ...
            distinct_items = sorted(list(set(item[column_name] for item in res.data if item[column_name] is not None)))
...
async def get_property_input_options():
    ...
    try:
        ...
        results = await asyncio.gather(
            ...
            supabase.from_(table_name).select("size_m2").not_.is_("size_m2", "null").order("size_m2", desc=False).limit(1).execute(), # <<< CORRECTED LINE
            supabase.from_(table_name).select("size_m2").not_.is_("size_m2", "null").order("size_m2", desc=True).limit(1).execute()  # <<< CORRECTED LINE
        )
...
MODEL_PATH = "C:\Users\user\Documents\Real Estate SPF\real-estate-system\real-estate-system\backend\models\property_price_model.joblib"

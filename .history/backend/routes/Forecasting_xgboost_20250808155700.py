...
if __name__ == "__main__":
    ...
    # Corrected split date for a 2-year test (2020 & 2021)
    ...
    if test_df.empty:
        raise ValueError("No data available for validation (post-2021). Cannot proceed.")
    print(f"data split for validation at: {split_date}")
    ...
    print("\n--- Starting GridSearchCV with new parameter grid ---")
    tscv = TimeSeriesSplit(n_splits=3)
    # Your new parameter grid
    param_grid = {
        'objective': 'reg:squarederror',
        'random_state': 42,
    ...
    model = xgb.XGBRegressor(objective='reg:squarederror', random_state=42, early_stopping_rounds=50, **best_params)
    model.fit(X_train, y_train, eval_set=[(X_test, y_test)], verbose=False)
    ...
    print(f"Mean Forecast MAPE: {mean_absolute_percentage_error(y_test, predictions) * 100:.2f}%")

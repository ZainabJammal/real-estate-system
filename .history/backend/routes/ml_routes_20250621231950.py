...
def map_user_selection_to_city(selection_string: str) -> str:
    """
    Maps the user's dropdown selection to the specific custom city names
    used in the database and the model.
    """
    # Use .lower() to make the check case-insensitive and robust
    lowered_selection = selection_string.lower()
    
    # --- THIS LOGIC NOW EXACTLY MATCHES YOUR DATABASE ---

    if "tripoli" in lowered_selection and "akkar" in lowered_selection:
        return "tripoli" # You named this region 'Tripoli'

    if "baabda" in lowered_selection and "aley" in lowered_selection:
        return "baadbad" # You named this region 'Baadbad'

    if "kesrouan" in lowered_selection and "jbeil" in lowered_selection:
        return "kesrouan" # You named this region 'Kesrouan'
        
    # For all other single-name cases (like "Beirut"), convert to lowercase
    # to match the standardized data in memory.

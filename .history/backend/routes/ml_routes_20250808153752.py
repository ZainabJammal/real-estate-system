...
def create_features(df):
    """
    *** THIS MUST BE AN EXACT COPY OF THE NEW TRAINING SCRIPT'S create_features ***
    """
    df_features = df.copy()
    df_features['year'] = df_features.index.year
    df_features['is_december'] = (df_features.index.month == 12).astype(int)
    ...
    df_features['yoy_growth'] = df_features.groupby(GROUPING_KEY)[TARGET_COL].pct_change(12).shift(1)
...
print("--- Forecasting API is ready. ---")@ml_routes.route("/forecast/xgboost/<string:user_selection>", methods=["GET"])
async def forecast_with_xgboost(user_selection: str):
    ...
    try:
        ...
        # Step C: Combine and create features
        combined_df = pd.concat([hist_df, future_df])
        combined_features = create_features(combined_df)

        # Step D: One-hot encode and align columns
        features_for_pred = combined_features.loc[future_df.index]
        features_encoded = pd.get_dummies(features_for_pred, columns=[GROUPING_KEY])
        features_aligned = features_encoded.reindex(columns=MODEL_FEATURES, fill_value=0)
        
        # Step E: Predict all 60 months in a single batch
        predictions = MEAN_MODEL.predict(features_aligned)
        
        # --- Step F: Format the response (CORRECTED) ---
        
        # --- FIX 1: Create the final forecast DataFrame with the CORRECT column name ---
        forecast_df = pd.DataFrame({
            'date': future_dates,
            'mean_forecast': predictions # Use the key the frontend expects
        })

        # --- FIX 2: Use .reset_index() so the date is included in the JSON ---
        historical_json = hist_df.reset_index().to_dict(orient='records')
        
        # Create monthly JSON from our corrected DataFrame
        monthly_json = forecast_df.to_dict(orient='records')

        # --- FIX 3: Resample correctly and use the CORRECT column name ---
        # We also need to set 'date' as the index on forecast_df to resample
        yearly_forecast_df = forecast_df.set_index('date').resample('YE')['mean_forecast'].sum().reset_index()
        yearly_forecast_df = yearly_forecast_df.rename(columns={'date': 'year', 'mean_forecast': 'mean_forecast'}) # Keep the name consistent
        yearly_forecast_df['year'] = yearly_forecast_df['year'].dt.year

        yearly_json = yearly_forecast_df.to_dict(orient='records')

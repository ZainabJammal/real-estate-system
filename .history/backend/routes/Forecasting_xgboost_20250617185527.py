...
# ==============================================================================
# 4. MAIN WORKFLOW: THE DEFINITIVE, MERGED VERSION
# ==============================================================================
if __name__ == "__main__":
    ...
    # One-hot encode the training set to get the feature list for tuning
    ...
    # Prepare the 2016 validation set, ensuring columns match the training set
    ...
    city_indices = y_validation.index[validation_final.get(f'city_{city_to_plot}', default=False)]
    ...
    # We must use the same feature set derived from the training data
    all_data_final = pd.get_dummies(data_with_features, columns=['city'], prefix='city').dropna()
    ...
    joblib.dump(production_model, MODEL_FILE_PATH)
    pd.Series(FEATURES).to_json(MODEL_COLS_PATH) # Save the exact feature list

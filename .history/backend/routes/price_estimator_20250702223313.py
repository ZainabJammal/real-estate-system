...
def prepare_apartment_data(df):
    ...
    # Isolate apartment data
    df_apartments = df[df['property_type'] == 'apartment'].copy()
    print(f"   -> Found {len(df_apartments)} initial apartment rows.")

    # Clean data: Replace 0s and handle missing values
    ...
    print(f"   -> {len(df_apartments)} rows after cleaning critical values.")
    ...
    print(f"   -> {len(df_apartments_clean)} rows after removing outliers.")

    # Final Transformation: Log transform the target variable
    df_apartments_clean['log_price'] = np.log1p(df_apartments_clean['price_$'])
    print("   -> Created 'log_price' as the final target variable.")
        # Insert this after your outlier removal step (after creating df_apartments_clean)

    print("\n--- Advanced Feature Engineering: Handling Rare Categories ---")

    # Calculate how many apartments are in each district
    district_counts = df_apartments_clean['district'].value_counts()
    print("   -> Original district counts:\n", district_counts)

    # Define a threshold for what is considered a 'rare' district
    # Let's say any district with fewer than 30 apartments is rare
    THRESHOLD = 30 
    rare_districts = district_counts[district_counts < THRESHOLD].index.tolist()
    print(f"\n   -> Districts identified as 'rare' (less than {THRESHOLD} samples): {rare_districts}")

    # Group these rare districts into a new 'Other' category
    # .loc is used to safely modify the DataFrame
    df_apartments_clean.loc[df_apartments_clean['district'].isin(rare_districts), 'district'] = 'Other'
    print("\n   -> District counts after grouping rare ones into 'Other':")
    print(df_apartments_clean['district'].value_counts())
    
    return df_apartments_clean


# --- 3. MODELING WORKFLOW ---
def build_and_evaluate_model(df_clean, config, output_dir):
...
# First, create one-hot encoded columns for the cleaned districts
# The prefix 'dist' helps keep column names clean
district_dummies = pd.get_dummies(df_apartments_clean['district'], prefix='dist', dtype=int)

# Now, create interaction features by multiplying size with each new district column
...
# You don't need to add the dummy columns themselves, just the interactions
# But first, let's update our feature list for the model

# Combine the new interaction features with your original features
new_interaction_features = [f'size_x_{col}' for col in district_dummies.columns]
    ...
    CATEGORICAL_FEATURES = ['province', 'district'] # Keep 'district' here for the pipeline to encode

    # Combine all features for the model
    # The new interaction columns are numerical, so we add them to that list
    UPDATED_NUMERICAL_FEATURES = NUMERICAL_FEATURES + new_interaction_features
    FEATURES = UPDATED_NUMERICAL_FEATURES + CATEGORICAL_FEATURES

    # You MUST make sure the interaction columns are present in the DataFrame fed to the model
    # One way to do this is to merge them
    df_apartments_final = pd.concat([df_apartments_clean, district_dummies], axis=1)

    # Now, when you define X, use this final DataFrame
    X = df_apartments_final[FEATURES]
    y = df_apartments_final[TARGET]
...
def plot_feature_importance(pipeline, output_dir):
    ...
    all_feature_names = np.concatenate([preprocessor.transformers_[0][2], cat_features_encoded])
...
# --- 5. MAIN EXECUTION ---
def main():
    """Main function to run the complete model training workflow."""
    
    # Step 1: Setup
    data_path, output_dir = setup_environment()

    # Load and scope initial data
    df = pd.read_csv(data_path)
    df.drop_duplicates(subset=['id'], inplace=True)
    df.rename(columns={'type': 'property_type'}, inplace=True)
    df['property_type'] = df['property_type'].str.lower().str.strip()
    ...
    df_clean = prepare_apartment_data(df)
    
    # Step 3 & 4: Build, validate, and train the model
    final_pipeline, avg_r2_score = build_and_evaluate_model(df_clean, MODEL_CONFIG['apartment'], output_dir)
    
    # Step 5: Plot feature importance
    plot_feature_importance(final_pipeline, output_dir)
    
    # Step 6: Save the final model
    model_path = os.path.join(output_dir, 'model_apartment.joblib')
    ...
    summary = {
        'Model': 'Apartment Price Estimator',
        'Average R2 (5-Fold CV)': f"{avg_r2_score:.4f}",
        'Number of Training Samples': len(df_clean),
        'Model Saved At': model_path,
        'Importance Plot Saved At': os.path.join(output_dir, 'feature_importance.png')
    }

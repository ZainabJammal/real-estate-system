...
async def forecast_with_xgboost(user_selection: str):
    ...
    try:
        # Step 1: Map user selection to the lowercase city name
         # Step 1: Create a DataFrame for ALL 60 future months
        last_hist_date = pd.to_datetime('today') # Or get the actual last date from DB
        future_dates = pd.date_range(start=last_hist_date + pd.DateOffset(months=1), periods=FORECAST_HORIZON_MONTHS, freq='MS')

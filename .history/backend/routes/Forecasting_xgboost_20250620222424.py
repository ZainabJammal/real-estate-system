...
# --- THIS FUNCTION IS COMPLETELY REWRITTEN ---
   Loads data, normalizes cities into regions using a mapping file,
    ...
    # --- Step 1: Normalize the raw city strings (same as before) ---
    df['city'] = df['city'].str.split(',')
    df = df.explode('city')
    df['city'] = df['city'].str.strip()

    # --- Step 2: Load the region mapping from the JSON file ---
    with open(mapping_filepath, 'r') as f:
        region_map = json.load(f)

    # --- Step 3: Create a "reverse map" for easy lookup (city -> region) ---
    reverse_map = {}
    for region_name, cities in region_map.items():
        if region_name == "_DEFAULT_":
            ...
            for city in cities:
                reverse_map[region_name.strip()] = region_name # Use the region name itself

    # --- Step 4: Apply the mapping to create the new 'region' column ---
    # We map the cleaned 'city' column to its corresponding region name
    df[REGION_COL] = df['city'].map(reverse_map)
    
    # Handle any cities not in the mapping file by making them their own region
    ...
    # Group by date and the new region, summing up the transaction values
...
if __name__ == "__main__":
    ...
    region_to_plot = 'Baabda/Aley/Chouf' # Use the name from your JSON file

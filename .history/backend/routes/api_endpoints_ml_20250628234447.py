...
endpoints_ml = Blueprint('endpoints_ml', __name__)
...
@endpoints_ml.route("/forecast/xgboost/<string:user_selection>", methods=["GET"])
...
@endpoints_ml.route("/predict_price", methods=["POST"])
...
# @ml_rendpoints_mloutes.route("/price_prediction", methods=["POST"])
# async def price_prediction():
#     """API endpoint for property price prediction using the hybrid model strategy."""
#     if not artifacts:
...
#         # 1. Add the crucial location_cluster feature
...
#         # 2. The Router: Select the correct model based on what was successfully loaded
#         if property_type == 'apartment' and 'model_apartment' in artifacts:
#             model_to_use = artifacts['model_apartment']
#         elif property_type == 'office' and 'model_office' in artifacts:
#             model_to_use = artifacts['model_office']
#         elif property_type == 'shop' and 'model_shop' in artifacts:
#             model_to_use = artifacts['model_shop']
#         else:
#             # For all other types, use the fallback
#             model_to_use = artifacts['model_general_fallback']
#             response_status = "low_confidence_range"

#         # 3. Make the prediction
#         prediction_log = model_to_use.predict(input_df)
#         prediction_dollars = np.expm1(prediction_log)[0]

#         # 4. Format the response based on the confidence level
#         if response_status == "high_confidence":
#             response_data = {
#                 "status": "high_confidence",
#                 "prediction": round(prediction_dollars, -3) # Round to nearest thousand
#             }
#         else: # low_confidence_range
#             # Based on our analysis of the model with R²=0.42, the MAE was ~223k
#             mae_fallback = 224000 

...
if __name__ == "__main__":
    ...
    # --- Step 4: Evaluate and Plot the Validation Results ---
    ...
    # Plot the results for a specific region to see how it looks
    region_to_plot = 'beirut'
    
    # Filter the original data (before one-hot encoding) for plotting
    plot_train_df = data_with_features[
        (data_with_features.index < split_date) & 
        (data_with_features[GROUPING_KEY] == region_to_plot)
    ]
    plot_test_df = data_with_features[
        (data_with_features.index >= split_date) & 
        (data_with_features[GROUPING_KEY] == region_to_plot)
    ]

        # Create a DataFrame from the predictions to make filtering easy
    predictions_df = pd.DataFrame({'prediction': predictions}, index=y_test.index)

    # Join the predictions back to the test features
    results_df = X_test.join(predictions_df)

    # Now, filter for the specific region we want to plot
    region_to_plot_df = results_df[results_df[f'{GROUPING_KEY}_{region_to_plot}'] == 1]

    # The predictions for our chosen region are now in the 'prediction' column
    region_predictions = region_to_plot_df['prediction']

    if not plot_test_df.empty and not region_predictions.empty:
        # Use the new plotting function from the training script
        plt.figure(figsize=(15, 7))
        plt.plot(plot_train_df.index, plot_train_df[TARGET_COL], label='Training Data', color='blue')
        plt.plot(plot_test_df.index, plot_test_df[TARGET_COL], label='Actual Values (2016)', color='green', marker='o')
        # Plot the correctly filtered predictions against their corresponding dates
        plt.plot(region_predictions.index, region_predictions.values, label='Predicted Values (2016)', color='red', linestyle='--')
        plt.title(f"'Best Guess' Model Validation for {region_to_plot.capitalize()}", fontsize=16)
        plt.xlabel('Date')
        plt.ylabel('Transaction Value')
        plt.legend()
        plt.grid(True)
        ...
        print(f"No validation data or predictions to plot for {region_to_plot}.")

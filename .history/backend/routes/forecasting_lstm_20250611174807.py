...
class ModelEvaluator:
    def __init__(self, granularity: str):
        ...
        self.artifacts_path = "backend\model_artifacts"
        
        # Load all the artifacts created during training
        self._load_artifacts()
        self.look_back = self.model.layers[0].input_shape[1] # Auto-detect look_back from model
    ...
    async def prepare_evaluation_data(self):
        ...
        response = await supabase.table('transactions').select("created_at, city, transaction_value").execute()
...
async def main():
    ...
    try:
        ...
        await evaluator.prepare_evaluation_data()
        evaluator.evaluate()

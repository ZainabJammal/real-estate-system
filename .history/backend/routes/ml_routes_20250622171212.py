...
async def forecast_with_xgboost(user_selection: str):
    ...
    try:
        ...
        # Get the year of the last historical data point.
        last_hist_year = hist_df['date'].iloc[-1].year
        
        # Explicitly set the start date to January 1st of the *next* year.
        start_year = last_hist_year + 1
        start_date = pd.to_datetime(f'{start_year}-01-01')
        
        print(f"-> Last historical year is {last_hist_year}. Starting forecast from {start_date.date()}.")

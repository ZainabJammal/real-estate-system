...
FORECAST_YEARS = 3  # Or 3, but keep it as a whole number

# --- Calculate the horizon automatically ---
FORECAST_HORIZON_MONTHS = FORECAST_YEARS * 12 # This will be 24
...
def create_features_for_prediction(df, city_avg_value, monthly_avg_map, last_time_index):
    df_features = df.copy()
    # last_time_index = df_features.index[-1]
...
async def forecast_with_xgboost(user_selection: str):
    ...
    try:
        # *** FIX WAS APPLIED IN THIS BLOCK ***
        print(f"-> Step 1: Querying Supabase for '{city_name}' historical data...")
        # FIX 1: Added GROUPING_KEY to the select query to get the city name column
        ...
        # FIX 2: All subsequent operations now correctly use the 'hist_df' DataFrame
        hist_df = pd.DataFrame(response.data)
        
        # Perform date conversion on the DataFrame
        parts = hist_df['date'].str.split('-', expand=True)
        hist_df['date'] = pd.to_datetime('01-' + parts[1] + '-' + parts[0], format='%d-%b-%y')
        hist_df.set_index('date', inplace=True)
        hist_df.sort_index(inplace=True)

        # This line now works because the GROUPING_KEY column was selected from the database
        ...
        # --- The rest of the logic continues from here ---

...
async def predict_apartment_price():
    ...
    try:
        ...
        price_range_high = prediction_dollars * 1.10
        
        response_data = {
            "status": "success",
            "estimated_price": int(round(prediction_dollars, -2)), 
            "price_range_low": int(round(price_range_low, -2)),   
            "price_range_high": int(round(price_range_high, -2)), 
        }

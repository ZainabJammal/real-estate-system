...
def train_and_evaluate_model():
    """
    V6: Professional workflow with all enhancements and a corrected, robust
    two-step tuning and training process.
    """
    ...
    y = np.log1p(df['price_$']) # Use log-transformed target from the start
    ...
    def objective(trial):
        ...
        model = Pipeline(steps=[('preprocessor', preprocessor), ('regressor', lgb.LGBMRegressor(**params))])
        
        kf = KFold(n_splits=5, shuffle=True, random_state=42)
        # *** REFINEMENT: Use early stopping inside the cross-validation for speed ***
        # This is a more advanced technique that makes tuning much faster.
        scores = []
        for train_index, val_index in kf.split(X_train):
            ...
            model.fit(X_train_fold, y_train_fold, 
                      regressor__eval_set=[(model.named_steps['preprocessor'].transform(X_val_fold), y_val_fold)],
    ...
    # *** FIX: Create the final pipeline and fit it on the FULL training data ***
    best_params = study.best_params
    final_pipeline = Pipeline(steps=[
        ('preprocessor', preprocessor),
        ('regressor', lgb.LGBMRegressor(objective='regression_l1', random_state=42, **best_params))
    ])
    ...
    # Convert back from log scale for evaluation in dollars
    ...
    model_filename = 'property_price_model_v6_final.joblib'
    print(f"\nStep 9: Saving final model to '{model_filename}'...")
    joblib.dump(final_pipeline, model_filename)
    joblib.dump(kmeans, 'location_cluster_model.joblib') # Also save the k-means model
    joblib.dump(district_price_per_m2, 'district_price_model.joblib') # And the district price averages

...
def create_features(df):
    ...
    # Binary flags (optimized for MS data)
    df_features['is_month_start'] = df_features.index.is_month_start.astype(int)  # Always 1 for MS data
    df_features['is_quarter_start'] = df_features.index.is_quarter_start.astype(int)  # Jan, Apr, Jul, Oct
    df_features['is_quarter_end'] = df_features.index.is_quarter_end.astype(int)  # Mar, Jun, Sep, Dec
    df_features['is_year_start'] = df_features.index.is_year_start.astype(int)  # Only January
    df_features['is_year_end'] = df_features.index.is_year_end.astype(int)  # Only December
...
if __name__ == "__main__":
   ...
   # --- Step 4: Train Model with Early Stopping and Evaluate on Validation Set ---
    print("\n--- Training model with best params and early stopping ---")
      ...
      # Correct: Pass eval_metric to the constructor
    ...
    df['weight'] = np.where(df.index.year >= 2017, 1.5, 1.0)

      # Correct: .fit() no longer has the eval_metric argument
    ...
    plt.show()
      ...
      # Residuals by city
    df_val['residuals'] = residuals
    sns.boxplot(x='city', y='residuals', data=df_val)

...
# def create_and_train_pipeline(X_train, y_train, X_test, y_test, params, feature_config):
#     """
#     Creates, trains, evaluates, and returns a model pipeline, ensuring feature names are handled correctly.
#     """
#     preprocessor = ColumnTransformer(
#         transformers=[
#             ('num', StandardScaler(), feature_config['numerical']),
...
#         ],
#         remainder='drop',
#         verbose_feature_names_out=False # Ensures clean feature names like 'district_Beirut' instead of 'cat__district_Beirut'
#     )
...
#     # --- THE FIX FOR FEATURE NAMES ---
#     # 1. Fit the preprocessor on the training data to learn the columns.
#     pipeline.named_steps['preprocessor'].fit(X_train)
    
#     # 2. Get the full list of feature names *after* one-hot encoding.
#     final_feature_names = list(pipeline.named_steps['preprocessor'].get_feature_names_out())

#     # 3. Fit the entire pipeline, passing the feature names to the LightGBM regressor.
...
#             joblib.dump(pipeline, model_path)
#             print(f"   -> High-quality model saved to '{model_path}'")
#             # Generate the plot for the saved model
...
#     joblib.dump(general_pipeline, model_path)
#     print(f"   -> Generalist Fallback Model saved to '{model_path}'")
#     # Generate the plot for the generalist model as well

...
def create_and_train_pipeline(X_train, y_train, X_test, y_test, params, feature_config):
    ...
    """
    # Define the preprocessor based on the feature configuration for this model
...
def main():
    ...
    # --- MODEL CONFIGURATION ---
    # This dictionary holds all the expert-tuned parameters and feature lists.
    MODEL_CONFIG = {
        'apartment': {
    ...
    df.drop(columns=['city', 'created_at'], inplace=True)
    ...
    for prop_type in specialist_types:
        ...
        X = df_slice.drop(columns=['price_$', 'type']) # Drop price and the now-constant type
        ...
        if r2 > 0.68:
            model_path = os.path.join('specialist_models', f'model_{prop_type}.joblib')
            joblib.dump(pipeline, model_path)
            print(f"   -> Model saved to '{model_path}'")
        else:
            print(f"   -> Model performance too low, will use Generalist Fallback.")

    # --- TRAINING GENERALIST FALLBACK MODEL ---
    print("\n--- Training Generalist Fallback Model ---")
    df_general = df[df['type'] != 'Land'].copy() # Exclude Land as it behaves too differently
    ...
    general_config = {
        ...
        'features': {
            ...
            'categorical': ['province', 'district', 'location_cluster', 'type'] # Must include 'type' here
        }
    }

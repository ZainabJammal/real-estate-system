...
def init_price_estimator_routes():
    ...
    try:
        ...
        # ### --- CORRECTED: Point to the final Supabase-trained model --- ###
        # Make sure this path is correct for your final deployment
        ...
        print(f"FATAL ERROR: Could not find the model file. {e}")
        # In a real app, you might want to return an error or disable the endpoint
    except Exception as e:
        print(f"FATAL ERROR during model loading: {e}")
        traceback.print_exc()
...
async def predict_apartment_price():
    ...
    try:
        ...
        response_data = {
            "status": "success",
            "estimated_price": round(prediction_dollars, -3), # Round to nearest 1000 for a cleaner look
            "price_range_low": max(0, round(price_range_low, -3)), # Ensure it's not negative

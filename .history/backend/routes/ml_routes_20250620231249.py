...
DATA_PATH = os.path.join(PROJECT_ROOT_DIR, 'data', 'transactions.csv')
CONFIG_PATH = os.path.join(PROJECT_ROOT_DIR, 'config', 'region_mapping.json')
MODELS_PATH = os.path.join(PROJECT_ROOT_DIR, 'models')

XGB_MODEL_PATH = os.path.join(MODELS_PATH, 'forecast_model.joblib')
XGB_COLS_PATH = os.path.join(MODELS_PATH, 'model_columns.json')
...
async def forecast_with_xgboost(city_name: str):
    """
    Generates a forecast for a given city by mapping it to its region and
    recursively predicting future values.
    """
    print(f"\nReceived XGBoost forecast request for city: {city_name}")
    try:
        # Step 1: Map the input city to its corresponding region
        region_name = CITY_TO_REGION_MAP.get(city_name)
        if not region_name:
            return jsonify({"error": f"City '{city_name}' not found in region mapping."}), 404

        print(f"-> Mapped '{city_name}' to region '{region_name}'.")

        # Step 2: Get the historical data for that entire REGION
        ...
        for date in future_dates:
            # Create a temporary dataframe for the future step
            temp_df = pd.DataFrame(index=[date], data={REGION_COL: region_name})
            
            # Combine history with the future step to calculate lags/rolling features
            combined_df = pd.concat([region_history, temp_df])
            
            # Create all features using the shared helper function
            ...
            # Add the prediction to the history so it can be used for the next step's lags
...
# ==============================================================================
#                 END OF MAYSNOUN'S REFACTORED BIT
# ==============================================================================
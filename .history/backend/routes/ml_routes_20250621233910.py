...
def map_user_selection_to_city(selection_string: str) -> str:
    ...
    lowercase city name used in the model.
    """
...
# Step B: Force the 'date' column to be a proper datetime object.
raw_df['date'] = pd.to_datetime(raw_df['date'])

# Step C: NOW, set the clean date column as the index.
region_history_df = raw_df.set_index('date')

# Step D: Clean up unnecessary columns from the database query
region_history_df = region_history_df.drop(columns=['id', 'created_at'], errors='ignore')

# Step E: Rename the 'region' column from the database to match the GROUPING_KEY ('city')
# This ensures consistency with the training script's feature creation.
region_history_df.rename(columns={'region': GROUPING_KEY}, inplace=True, errors='ignore')

# Step F: *** YOUR CRUCIAL CORRECTION ***
# Standardize the region names to lowercase to ensure a perfect match.
region_history_df[GROUPING_KEY] = region_history_df[GROUPING_KEY].str.lower()
...
async def forecast_with_xgboost(user_selection: str):
    ...
    try:
        ...
        # Step 2: Fetch historical data FOR THAT CITY ONLY from Supabase
        ...
        city_history_df = pd.DataFrame(response.data)
        city_history_df['date'] = pd.to_datetime(city_history_df['date'])
        city_history_df = city_history_df.set_index('date')

        # --- THIS IS THE FIX ---
        # Ensure the index is a proper DatetimeIndex object
        city_history_df.index = pd.to_datetime(city_history_df.index)

        # Ensure the column name is what the model expects
        city_history_df.rename(columns={'city': GROUPING_KEY}, inplace=True, errors='ignore')

        # Step 3: The robust recursive forecasting loop
        future_df_dynamic = city_history_df.copy()
        last_date = future_df_dynamic.index.max() # <-- This will now work correctly


        # # Convert query result to a clean DataFrame
        # city_history_df = pd.DataFrame(response.data)
        # parts = city_history_df['date'].str.split('-', expand=True)
        # city_history_df['date'] = pd.to_datetime('01-' + parts[1] + '-' + parts[0], format='%d-%b-%y')
        # ALL_HISTORICAL_DATA = city_history_df.set_index('date')
        # # city_history_df['date'] = pd.to_datetime(city_history_df['date'])
        # # city_history_df = city_history_df.set_index('date').drop(columns=['id', 'created_at'], errors='ignore')
        # # Ensure the column name is what the model expects
        # city_history_df.rename(columns={'city': GROUPING_KEY}, inplace=True, errors='ignore')

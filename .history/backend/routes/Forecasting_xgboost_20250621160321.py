...
# In your TRAINING SCRIPT (Forecasting_xgboost.py)

# This function becomes the "single source of truth"
def map_city_to_region(city_name):
    """
    This function defines the regions. It takes a single city name
    and returns the name of the region it belongs to.
    """
    
    # Check for the "North" region
    if city_name in ["Tripoli", "Akkar"]:
        return "North"  # <-- Return the string "North"
        
    # Check for the "Mount Lebanon South" region
    if city_name in ["Baabda", "Aley", "Chouf"]:
        return "Mount Lebanon South"
        
    # Check for the "Mount Lebanon North" region
    if city_name in ["Kesrouan", "Jbeil", "Metn"]:
        return "Mount Lebanon North"

    # If the city is not in any of the groups above,
    # it becomes its own region. The function returns the city's own name.
    # For example, if city_name is "Beirut", it will return "Beirut".
    # if city_name is "Bekaa", it will return "Bekaa".
    return city_name
...
if __name__ == "__main__":
    ...
    region_to_plot = 'Tripoli/Akkar' # Use the name from your JSON file

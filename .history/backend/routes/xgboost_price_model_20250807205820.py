...
#     # --- FIX: Convert columns to numeric types before using them ---
#     # This will turn any non-numeric values (like empty strings) into NaN
#     df_apartments['bedrooms'] = pd.to_numeric(df_apartments['bedrooms'], errors='coerce')
#     df_apartments['bathrooms'] = pd.to_numeric(df_apartments['bathrooms'], errors='coerce')
#     df_apartments['price_$'] = pd.to_numeric(df_apartments['price_$'], errors='coerce')
#     df_apartments['size_m2'] = pd.to_numeric(df_apartments['size_m2'], errors='coerce')
...
#     # 3. Interaction Feature (This will now work correctly)
...
#     if df_raw.empty:
#         print("\nNo data fetched from Supabase. Exiting.")
#         return

#     df_raw.drop_duplicates(subset=['id'], inplace=True)
#     df_raw.rename(columns={'type': 'type'}, inplace=True)
...
# --- 1. SETUP & CONFIGURATION ---
def setup_environment():
    """Sets up directories and configurations for the final model script."""
    warnings.filterwarnings('ignore', category=UserWarning)
    pd.options.mode.chained_assignment = None

    # Define a clear output directory for the final model
    output_dir = os.path.join(os.getcwd(), 'final_production_model')
...
def prepare_apartment_data(df):
    ...
    print("\n--- Step 2: Preparing Apartment Data for Production Model ---")
    ...
    # Basic cleaning and imputation prep
    df_apartments['bedrooms'].replace(0, np.nan, inplace=True)
    df_apartments['bathrooms'].replace(0, np.nan, inplace=True)
    df_apartments.dropna(subset=['price_$', 'size_m2'], inplace=True)
    df_apartments = df_apartments[df_apartments['size_m2'] > 0]
    ...
    # Feature Engineering
    df_apartments['price_per_m2'] = df_apartments['price_$'] / df_apartments['size_m2']
    df_apartments['bath_bed_ratio']= df['bedrooms'] / df['bathrooms']
    df_apartments['size_per_bedroom'] = df_apartments['size_m2'] / df_apartments['bedrooms']
    
    df_apartments['avg_price_in_district']= df.groupby('district')['price_per_m2'].transform('mean')
    df_apartments['avg_size_in_district'] = df.groupby('district')['size_m2'].transform('mean')

    # Outlier removal based on price per square meter
    q_low = df_apartments['price_per_m2'].quantile(0.01)
    q_high = df_apartments['price_per_m2'].quantile(0.99)
    df_apartments_clean = df_apartments[(df_apartments['price_per_m2'] >= q_low) & (df_apartments['price_per_m2'] <= q_high)].copy()

    # Consolidate rare districts to prevent overfitting
    district_counts = df_apartments_clean['district'].value_counts()
    THRESHOLD = 10
    rare_districts = district_counts[district_counts < THRESHOLD].index.tolist()
    if rare_districts:
...
# --- (Plotting functions can remain the same) ---
def plot_regression_results(y_true, y_pred, output_dir):
    """Generates a scatter plot of actual vs. predicted values."""
    print("\n--- Generating Final Regression Plot ---")
    plt.figure(figsize=(10, 8))
    plt.scatter(y_true, y_pred, alpha=0.4)
    perfect_line = np.linspace(min(y_true.min(), y_pred.min()), max(y_true.max(), y_pred.max()), 100)
    plt.plot(perfect_line, perfect_line, color='red', linestyle='--', linewidth=2, label='Perfect Prediction')
    ...
    plt.title("Actual vs. Predicted Prices (Final XGBoost Model)")
    plt.legend()
    plt.gca().ticklabel_format(style='plain', axis='both')
    plt.grid(True)
    plt.tight_layout()
    plot_path = os.path.join(output_dir, 'final_regression_plot.png')
    plt.savefig(plot_path)
    plt.close()
    print(f"   -> Regression plot saved to '{plot_path}'")

def plot_feature_importance(pipeline, output_dir):
    """Extracts and plots feature importances from the final model."""
    ...
    plt.tight_layout()
    plot_path = os.path.join(output_dir, 'final_feature_importance.png')
    plt.savefig(plot_path)
    plt.close()
    print(f"   -> Feature importance plot saved to '{plot_path}'")
...
def main():
    ...
    # MODIFIED: Fetch data from Supabase instead of a local CSV
    df_raw = fetch_supabase_data()
    if df_raw.empty:
    ...
    df_raw.drop_duplicates(subset=['id'], inplace=True)
    # The 'type' column name is already correct in the CSV.

    df_featured = prepare_apartment_data(df_raw)

    # --- DEFINE FEATURES AND TARGET (Matched to Front-End) ---
    TARGET = 'log_price'
    NUMERICAL_FEATURES = ['log_size_m2', 'bedrooms', 'bathrooms', 'latitude', 'longitude']
    # 'city' is now included as a categorical feature
    CATEGORICAL_FEATURES = ['province', 'district', 'city']
    FEATURES = NUMERICAL_FEATURES + CATEGORICAL_FEATURES
    ...
    # --- Split Data: 70% for Training, 30% for Final Testing ---
    print("\n--- Step 3: Splitting Data into Training (70%) and Testing (30%) Sets ---")
    X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=0.3, random_state=42)
    print(f"   -> Training set size: {len(X_train)} rows")
    ...
    model_path = os.path.join(output_dir, 'final_apartment_price_model.joblib')
    joblib.dump(best_xgb_model, model_path)
    ...
    print(f"   -> Best Cross-Validation R² (on train set): {grid_search_xgb.best_score_:.4f}")

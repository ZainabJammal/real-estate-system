...
if __name__ == "__main__":
   ...
   # --- Step 7: Create the Unified Forecast Visualization (FINAL CORRECTED VERSION) ---
...
if not city_test_df.empty:
    ...
    # **THE FIX IS HERE**: We use `.values` to turn the pandas boolean Series
    # into a raw NumPy boolean array, which correctly filters the prediction arrays.
    city_mean_preds = mean_preds[city_indices_mask.values]
    city_p10_preds = p10_preds[city_indices_mask.values]
    city_p90_preds = p90_preds[city_indices_mask.values]
    ...
    # Plot the actual values from the test set (x-axis has 12 points)
    plt.plot(city_test_df.index, city_test_df[TARGET_COL], label='Actual Values', color='green', marker='o', linestyle='-')
    
    # Plot the model forecasts (y-axes now also have 12 points)

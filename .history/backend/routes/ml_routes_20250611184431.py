...
async def forecast_route():
    ...
    try:
        ...
        future_periods = data.get('periods', 12)
        ...
        db_date_col = "created_at"
        db_value_col = "transaction_value"
        db_city_col = "city"

        print(f"[DEBUG] Querying table 'transactions' for city '{city_name}'...")
        response = supabase.table('transactions').select(f"{db_date_col}, {db_value_col}").eq(db_city_col, city_name).order(db_date_col, desc=False).execute()

...
def init_price_estimator_routes():
    ...
    global PIPELINE  # Declare that we are modifying the global PIPELINE variable

    try:
        # --- Correctly navigate to the backend directory to find the model ---
        script_path = os.path.abspath(__file__)
        routes_dir = os.path.dirname(script_path)
        backend_dir = os.path.dirname(routes_dir)
        ...
        # The PIPELINE will remain None, and the endpoint will fail gracefully.
    except Exception as e:
        print(f"FATAL ERROR: An unexpected error occurred during model loading: {e}")
        traceback.print_exc()
    
    # --- 4. DEFINE THE API ENDPOINT ---
    @price_estimator_bp.route("/predict/apartment", methods=["POST"])
    async def predict_apartment_price():
        ...
        try:
            ...
            # --- b. PERFORM FEATURE ENGINEERING (CRITICAL STEP) ---
            # The input data MUST be transformed to match the training data format.
            ...
            # Group rare districts into 'Other'
            # NOTE: We must use the SAME list of rare districts from training.
            # It's better to save this list or hardcode it.
            # For now, we assume the input district is one of the major ones.
            # A more advanced version would save the list of valid districts from training.
            
            # Create interaction features
            # This is the trickiest part. The model expects columns for ALL districts it was trained on.
            # We need to create these columns and set the correct one to 1.
            
            # --- A Simpler, More Robust Approach ---
            # The pipeline already handles one-hot encoding! We just need to ensure
            # the input DataFrame has the right columns before the interaction features.
            
            # Create interaction features for all possible districts from the training phase
            # This list should be saved from your training script, but we can hardcode it for now
            trained_districts = ['Beirut', 'El Metn', 'Jbeil', 'Baabda', 'Kesrouane', 'Other']
            
            user_district = input_df['district'].iloc[0]
            
            for dist in trained_districts:
                interaction_col_name = f'size_x_dist_{dist}'
                # If the user's district matches this column, calculate the interaction. Otherwise, it's 0.
                if dist == user_district:
            ...
            # The pipeline will handle scaling and one-hot encoding of the base features
            ...
            response_data = {
                "status": "success",
                "predicted_price_$": round(prediction_dollars, -3) # Round to nearest thousand
            }

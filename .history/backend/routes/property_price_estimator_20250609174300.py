...
if __name__ == "__main__":
    try:
        # Build paths relative to this script's location for reliability
        # __file__ is the path to the current script (e.g., .../backend/models/property_price_estimator.py)
        # os.path.dirname(__file__) is the directory of the script (.../backend/models)
        
        # Path to the models directory
        models_dir = os.path.dirname(__file__) 
        # Path to the backend directory (one level up)
        backend_dir = os.path.dirname(models_dir)
        # Path to the project root (one level up from backend)
        project_root = os.path.dirname(backend_dir)
        
        data_path = os.path.join(project_root, "Schema", "properties.csv")
        model_save_path = os.path.join(models_dir, "property_price_model.joblib")
        ...
        print(f"❌ CRITICAL ERROR: Could not find the dataset at '{data_path}'.")

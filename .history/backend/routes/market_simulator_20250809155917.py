...
@market_simulator_bp.route("/market-simulator/<string:user_selection>", methods=["GET"])
async def get_market_scenarios(user_selection:):
    """
    Returns market scenarios for a given city selection.
    e.g., /api/market-simulator?selection=Beirut
    """
    # Step 1: Get user's selection from the request query parameters
    user_selection = request.args.get('selection')
    if not user_selection:
        return jsonify({"error": "Missing 'selection' parameter in request."}), 400

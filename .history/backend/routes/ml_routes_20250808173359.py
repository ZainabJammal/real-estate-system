...
async def forecast_with_xgboost(user_selection: str):
    ...
    try:
        ...
        historical_json = hist_df.to_dict(orient='records')
        monthly_forecast_df= pd.DataFrame({'date': future_dates, 'mean_forecast': predictions}).to_dict(orient='records')
        yearly_forecast_df = monthly_forecast_df.DataFrame({'date': future_dates, 'mean_forecast': predictions}).set_index('date').resample('YE')['mean_forecast'].sum().reset_index()

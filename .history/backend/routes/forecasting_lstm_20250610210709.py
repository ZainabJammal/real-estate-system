...
async def fetch_and_prepare_transaction_data(
    ...
    try:
        ...
        print(f"DEBUG: Successfully parsed 'date' column with format %d %m/%Y. Null count after parse: {df_raw['date'].isnull().sum()}")
    except ValueError as ve: # Catch specific error if format fails
        print(f"WARNING: Failed to parse 'date' with format %d %m/%Y (Error: {ve}). Attempting generic parsing with errors='coerce'.")

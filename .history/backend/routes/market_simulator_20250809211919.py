...
def boom_scenario_regional(df):
    """Applies a region-specific 'boom' trend on top of the baseline data."""
    df_boom = df.copy()

    def calculate_boom(group):
        city_name = group.name
        # Get the specific annual growth rate for this city, or use default
        annual_growth = SCENARIO_RATES.get(city_name, SCENARIO_RATES['default'])['boom']
        m = (1 + annual_growth) ** (1/12) # Monthly multiplier
...
async def get_market_scenarios():
    ...
    boom = boom_scenario_regional(base_df)
    crash = crash_scenario_regional(base_df)

...
def create_features(df):
    ...
    # Lag features (for recursive forecasting)
    for lag in [1, 2, 3]:
        df[f'lag_{lag}'] = df['transaction_value'].shift(lag)
    
    # Rolling stats
    df['rolling_mean_3'] = df['transaction_value'].rolling(3).mean()
    df['rolling_std_3'] = df['transaction_value'].rolling(3).std()

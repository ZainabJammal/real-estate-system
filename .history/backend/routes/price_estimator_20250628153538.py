...
# (Helper functions 'plot_feature_importance' and 'create_and_train_pipeline' remain the same)
def plot_feature_importance(pipeline, prop_type):
    regressor = pipeline.named_steps['regressor']
    if not hasattr(regressor, 'feature_importances_') or not hasattr(regressor, 'feature_name_'):
        print(f"   -> Could not generate plot for {prop_type}, model lacks feature importance attributes.")
...
def main():
    ...
    # --- TRAINING SPECIALIST MODELS ---
    specialist_types = ['apartment', 'office', 'shop']
    # This list will hold all types NOT trained by a specialist
    generalist_types_to_train = list(df['type'].unique())
    ...
    for prop_type in specialist_types:
        ...
        if len(df_slice) < 50:
            print(f"   -> Insufficient data, this type will be handled by the generalist model.")
            continue
        
        # If we are training a specialist, remove it from the generalist's to-do list
        if prop_type in generalist_types_to_train:
            generalist_types_to_train.remove(prop_type)
            
        config = MODEL_CONFIG[prop_type]
        # Ensure all columns exist, fill with 0 if they don't (e.g., bedrooms in shops)
        for col in config['features']['numerical'] + config['features']['categorical']:
            if col not in df_slice.columns:
                df_slice[col] = 0
        ...
        if r2 > 0.65:
            ...
            print(f"   -> Model performance below threshold. This type will use the Generalist Fallback.")
            # Add it back to the generalist list if it failed
            generalist_types_to_train.append(prop_type)


    # --- TRAINING GENERALIST FALLBACK MODEL ---
    print("\n--- Training Generalist Fallback Model ---")
    print(f"The following types will be handled by the generalist model: {generalist_types_to_train}")
    
    df_general = df[df['type'].isin(generalist_types_to_train)].copy() 

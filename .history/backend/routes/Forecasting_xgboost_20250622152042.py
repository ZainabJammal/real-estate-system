...
if __name__ == "__main__":
    ...
    # all_data = load_data_from_supabase() # Use this for the Supabase version
    all_data = load_and_preprocess_data(CSV_FILE_PATH) # Use this for the CSV version
    
    # Use the optimal create_features function (NO LAGS)
    data_with_features = create_features(all_data)
    ...
    # --- Step 3: Define Optimal Hyperparameters and Train the Model ---
    ...
    # Use the test set for early stopping to prevent overfitting
    model.fit(X_train, y_train, eval_set=[(X_test, y_test)], early_stopping_rounds=50, verbose=False)

    # --- Step 4: Evaluate and Plot Validation Results ---
    ...
    # Filter the X_test to get data only for the region we want to plot
    X_validation_region = X_test[X_test[f'{GROUPING_KEY}_{region_to_plot}'] == 1]

    if not X_validation_region.empty:
        # Predict ONLY on this region's data
        ...
        plot_validation_results(
            ...
            region_name=region_to_plot.capitalize()
        )
    else:
    ...
    # Use the same robust hyperparameters for the final model
    # Note: We use .best_iteration from the early stopping to train the perfect number of trees

...
#             print(f"F irst record received from Supabase: {response.data[0]}") 
#         else:
...
XGB_MODEL  = joblib.load(MODEL_PATH)
XGB_MODEL_COLS = pd.read_json(MODEL_COLS_PATH, typ='series').tolist()
print("--- Forecasting API is ready. ---")

@ml_routes.route("/forecast/xgboost/<string:user_selection>", methods=["GET"])
async def forecast_with_xgboost(user_selection: str):
    print(f"\nReceived final forecast request for: '{user_selection}'")
    ...
    try:
        ...
        # Step A: Fetch historical data (only used for the historical part of the plot)
        if not response.data:
        ...
        city_history_df = hist_df.set_index('date', inplace=True)

        print("-> Historical DataFrame prepared successfully.")
        city_history_df[GROUPING_KEY] = city_history_df[GROUPING_KEY].str.lower()


        # Step B: Create a DataFrame for all 60 future months
        last_date = hist_df['date'].iloc[-1].year
        start_year = last_date + 1
        start_date = pd.to_datetime(f'{start_year}-01-01')
        future_dates = pd.date_range(start=start_date + pd.DateOffset(months=1), periods=60, freq='MS')
        future_df = pd.DataFrame(index=future_dates)
        future_df[GROUPING_KEY] = city_name

        # Step C: Create features DIRECTLY on the future DataFrame
        features_for_pred = create_features(future_df)
        
        # Step D: One-hot encode and align
        features_encoded = pd.get_dummies(features_for_pred, columns=[GROUPING_KEY])
        features_aligned = features_encoded.reindex(columns=MODEL_COLS_PATH, fill_value=0)
        
        # Step E: Predict all 60 months in a single batch
        predictions = MODEL_PATH.predict(features_aligned)

        # Step F: Format the response
        forecast_df = pd.DataFrame({'date': future_dates, 'mean_forecast': predictions})
        historical_json = hist_df.to_dict(orient='records')
        monthly_json = forecast_df.to_dict(orient='records')

        yearly_df = forecast_df.set_index('date').resample('YE')['mean_forecast'].sum().reset_index()
        yearly_df['year'] = yearly_df['date'].dt.year
        yearly_json = yearly_df.to_dict(orient='records')

...
def generate_future_forecasts(city_name: str, historical_data: pd.DataFrame) -> pd.DataFrame:
    ...
    *** THIS VERSION CONTAINS THE FIX FOR THE "FLAT PLOT" PROBLEM ***
    """
    future_predictions = []
    ...
    for i in range(FORECAST_HORIZON_MONTHS):
        ...
        # A. *** THE CRITICAL FIX IS HERE ***
        # We create a temporary placeholder row for the future date.
        # This ensures that when we call `create_features`, the time-based features
        # (year, month_sin, etc.) are calculated for the *correct future date*.
        ...
        # B. Prepare the single feature row that we will feed to the model
        ...
        X_future = future_row_aligned.iloc[-1:] # Select only the last row (our future date)

        # C. Predict using the mean model
        ...
        # E. CRITICAL: Feed the new prediction back into our history for the next loop

...
# # --- 4. API ENDPOINT (New "Best Possible Guess" Strategy) ---
# @ml_routes.route("/forecast/xgboost/<string:user_selection>", methods=["GET"])
# async def forecast_with_xgboost(user_selection: str):
#     """
#     Generates a 5-year 'Best Guess' forecast. This is not a recursive
#     time-series forecast, but a more stable prediction based on time features.
#     """
#     print(f"\nReceived 'Best Guess' forecast request for: '{user_selection}'")
#     try:
#         # Step 1: Map user selection to the model's city name
#         city_name = map_user_selection_to_city(user_selection)
#         print(f"-> Mapped to city: '{city_name}'")

#         # --- Part A: Generate the 5-Year Forecast ---

#         # Step 2: Create a DataFrame for all 60 future months
#         # We start the forecast from the first day of the next month
#         start_date = (pd.to_datetime('today').replace(day=1) + pd.DateOffset(months=1))
#         future_dates = pd.date_range(start=start_date, periods=FORECAST_HORIZON_MONTHS, freq='MS')
#         future_df = pd.DataFrame(index=future_dates)
        
#         # Step 3: Create features for this future DataFrame
#         # This uses the NEW, simple create_features function (without lags)
#         future_df[GROUPING_KEY] = city_name
#         features_for_pred = create_features(future_df)
        
#         # Step 4: One-hot encode the city and align columns perfectly with the model
...
#         # Step 5: Make predictions for ALL 60 months in a single batch
#         predictions = XGB_MODEL.predict(features_aligned)
#         print(f"-> Generated {len(predictions)} future predictions in a single batch.")

#         # --- Part B: Fetch Historical Data (Just for Display) ---
        
#         print(f"-> Querying Supabase for '{city_name}' historical data to display...")
#         response = supabase.table('agg_trans').select('*').ilike(GROUPING_KEY, city_name).order('date').execute()
...
#             # Use your trusted date parsing logic for the historical dates
#             parts = hist_df['date'].str.split('-', expand=True)
#             # This logic assumes the original Mon-YY format, e.g., 'Apr-11'
#             date_str_series = '01-' + parts[0] + '-' + parts[1]
#             hist_df['date'] = pd.to_datetime(date_str_series, format='%d-%b-%y')
#             # Convert to JSON for the frontend
#             historical_json = hist_df.to_dict(orient='records')
#         else:
#             print(f"Warning: No historical data found for {city_name} to display.")
...
#         print(f"-> Successfully generated 'Best Guess' forecast for '{city_name}'.")
#         return jsonify({
#             "city_forecasted": user_selection, # Send back the original selection
#             "historical_data": historical_json,
#             "monthly_forecast": monthly_json,

...
async def forecasting_transaction():
    try:
        ...
        return jsonify({"error": str(e)}), 500
async def fetch_distinct_values(supabase_client, column_name, table_name="properties"):
    try:
        res = await supabase_client.from_(table_name) \
                            .select(column_name) \
                            .neq(column_name, "is.null") \
                            ...
                            .execute()
        if res.data:
            return sorted(list(set(item[column_name] for item in res.data if item[column_name] is not None and str(item[column_name]).strip() != "")))
        return []
    except Exception as e:
        print(f"Error fetching distinct values for {column_name} from {table_name}: {e}")
        return []

async def fetch_numerical_options(supabase_client, column_name, table_name="properties", is_int=True):
    try:
        res = await supabase_client.from_(table_name) \
                            .select(column_name) \
                            .neq(column_name, "is.null") \
        ...
        if res.data:
            ...
            else: # float, for size_sqm range
                return sorted(list(set(float(v) for v in values)))
        return []
    except Exception as e:
...
async def get_property_input_options():
    ...
    try:
        ...
        cities = await fetch_distinct_values(supabase, "city")
        districts = await fetch_distinct_values(supabase, "district")
        provinces = await fetch_distinct_values(supabase, "province")
        types = await fetch_distinct_values(supabase, "type")
        
        # For size_sqm, get min/max
        size_values = await fetch_numerical_options(supabase, "size_sqm", is_int=False)
        size_range = {"min": min(size_values) if size_values else 0, "max": max(size_values) if size_values else 500} # Default max if no data

        # For bedrooms/bathrooms, get distinct counts
        ...
        if supabase:
            # If your create_supabase() returns a client that needs closing:
            # await supabase.postgrest.aclose() or similar
            pass

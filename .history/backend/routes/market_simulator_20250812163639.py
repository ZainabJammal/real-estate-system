...
def fetch_and_prepare_all_data():
    ...
    initial_df = pd.DataFrame(response.data)
    initial_df['date'] = pd.to_datetime(initial_df['date'])
    initial_df.rename(columns={'transaction_value': 'price'}, inplace=True)
    initial_df.sort_values(by=['city', 'date'], inplace=True)
...
def crash_scenario_stochastic(df, crash_start="2019-10-01"): # Updated crash start date to be more realistic
...
async def get_market_scenarios():
    ...
    print(f"-> Generating STOCHASTIC REGION-AWARE scenarios for '{city_name}'...")
    
    baseline = base_df.copy()
    baseline["scenario"] = "Baseline"
    
    # --- CALL THE NEW, STOCHASTIC FUNCTIONS ---
    boom = boom_scenario_stochastic(base_df)
    # --- THIS NOW MATCHES THE CORRECTED FUNCTION NAME ---

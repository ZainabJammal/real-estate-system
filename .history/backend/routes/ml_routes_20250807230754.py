...
def generate_future_forecasts(city_name: str, historical_data: pd.DataFrame) -> pd.DataFrame:
    """
    Generates a 5-year forecast iteratively using the trained models.
    """
    future_predictions = []
    
    # Use the last 13 months of historical data to correctly seed the lag calculations
    ...
    for i in range(FORECAST_HORIZON_MONTHS):
        ...
        # A. Create a temporary row for the next date to generate its features
        ...
        X_future = future_row_aligned.iloc[-1:] # Select the last row
        ...
        # D. Store the complete results for this month
        future_predictions.append({
            'date': next_date,
            'mean_forecast': float(pred_mean), # Cast to native Python float for JSON
            'p10_forecast': float(pred_p10),
            'p90_forecast': float(pred_p90)
        })

        # E. CRITICAL: Feed the MEAN prediction back into our data history for the next loop

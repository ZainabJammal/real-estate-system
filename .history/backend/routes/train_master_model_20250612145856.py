...
LOOK_BACK = 5      # Use 12 periods (months/years) of data to predict the next one.
GRANULARITY = 'Y'   # 'M' for Monthly, 'Y' for Yearly.
...
async def fetch_and_prepare_data(supabase_client: Client, granularity: str):
    ...
    This version assumes the date column in Supabase is named 'date'.
    """
    print(f"Fetching data for all cities with granularity: {granularity}...")
    
    # --- FIX 1: Use the correct column names in the .select() statement ---
    # We select 'date', 'city', and 'transaction_value'.
    response = await create supabase.table('transactions').select(
    ...
    # --- FIX 2: Convert the existing 'date' column from text to datetime objects ---
    # Even though the column is named 'date', it's still text from the JSON response.
    # We overwrite the existing column with the proper datetime format.
    print("Converting 'date' text column to datetime objects...")
    df['date'] = pd.to_datetime(df['date'])
    
    # We no longer need to create a new column or drop an old one.
    
    # This line will now work because the 'date' column exists and is in the correct format.
...
if __name__ == '__main__':
    ...
    if 'TRAINING_CONFIG' in locals():
        ...
        print("Invalid granularity argument. Please use 'M' for Monthly or 'Y' for Yearly.")

# (Removed duplicate block to avoid using TRAINING_CONFIG before assignment)
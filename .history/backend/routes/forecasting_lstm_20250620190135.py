...
if __name__ == "__main__":
    ...
    # To get predictions for just one city, we need to find its corresponding sequences in X_val_seq
    city_val_indices = [i for i, idx in enumerate(validation_final.index) if idx in validation_city_df.index]
    
    # Create sequences specifically for this city's validation data
    city_X_val_scaled = feature_scaler.transform(validation_final.loc[validation_city_df.index])
    city_y_val_unscaled = validation_city_df[TARGET_COL]
    city_X_val_seq, _ = create_lstm_sequences(city_X_val_scaled, city_y_val_unscaled, LOOK_BACK)
    
    city_plot_predictions_scaled = model.predict(city_X_val_seq)
    city_plot_predictions = target_scaler.inverse_transform(city_plot_predictions_scaled).flatten()

    # The predictions will have `look_back` fewer items than the validation set. Align them.
    plot_validation_results(train_city_df, validation_city_df.iloc[LOOK_BACK:], city_plot_predictions)

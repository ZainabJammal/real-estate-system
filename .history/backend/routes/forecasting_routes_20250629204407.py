...
def init_forecasting_routes(model_dir):
    ...
    try:
        ...
        XGB_MODEL['forecast_model'] = joblib.load(os.path.join(model_dir, 'forecast_model.joblib'))
        XGB_MODEL_COLS['forecast_columns'] = list(pd.read_json(os.path.join(model_dir, 'model_columns.json'), typ='series'))
...
async def forecast_with_xgboost(user_selection: str):
    ...
    try:
        ...
        predictions = FORECAST_MODELS.predict(features_aligned)

...
if __name__ == "__main__":
    print("--- Starting 'Best Guess' Model Training ---")

    # Load data from the database
    all_data = load_data_from_supabase()
    # Create the NEW, simpler feature set
    data_with_features = create_features(all_data)

    # One-Hot Encode the grouping key
    ...
    print(f"-> Training model on {len(X_all)} rows with features: {FEATURES}")

    # Use a robust set of hyperparameters - no need for a long GridSearchCV
    best_params = {
    ...
    production_model = xgb.XGBRegressor(**best_params)
    production_model.fit(X_all, y_all)

...
#     try:
#         # step A: generate next 5 years of forecast
...
#         # #     print(f"Warning: No historical data found for {city_name} to display.")
        ...
        #     # Step 1: Convert the 'date' column. Invalid dates become NaT.
        #     hist_df['date'] = pd.to_datetime(hist_df['date'], errors='coerce')
            
        #     # Step 2: Drop any rows from the DataFrame where the 'date' is NaT.
        #     # This is the crucial fix.
        #     hist_df.dropna(subset=['date'], inplace=True)
            
        #     # Now, hist_df is guaranteed to have no invalid dates.
        #     historical_json = hist_df.to_dict(orient='records')
        # else:
        #     print(f"Warning: No historical data found for {city_name} to display.")
...
async def forecast_with_xgboost(user_selection: str):
    """
    Generates a 5-year 'Best Guess' forecast that starts immediately
    after the last available historical data point.
    """
    print(f"\nReceived 'Best Guess' forecast request for: '{user_selection}'")
    try:
        city_name = map_user_selection_to_city(user_selection)
        ...
        # --- Part A: Fetch Historical Data FIRST ---
        ...
        # --- Part B: Generate the 5-Year Forecast ---
        ...
        # Make predictions for ALL 60 months at once
        predictions = XGB_MODEL.predict(features_aligned)

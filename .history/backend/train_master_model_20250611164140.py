...
async def fetch_and_prepare_data(supabase_client: Client, granularity: str):
    ...
    This version uses a more robust method to prevent dates from becoming column names.
    """
    print(f"Fetching data for all cities with granularity: {granularity}...")
    
    # IMPORTANT: Use the correct column names from your database here.
    # I am assuming they are 'created_at', 'city', and 'transaction_value' based on your error.
    response = supabase_client.table('transactions').select("created_at, city, transaction_value").execute()
    ...
    # Set 'date' as the index first, then group by 'city' and resample.
    # This is less prone to errors than the groupby().apply() chain.
    df_grouped = df.set_index('date').groupby('city')['transaction_value'].resample(granularity).sum()
    df_resampled = df_grouped.reset_index()

    # --- Add a debugging print statement to check the columns ---
    print(f"Columns after resampling: {df_resampled.columns.tolist()}") # DEBUG
    ...
    print(f"Columns after one-hot encoding: {df_encoded.columns.tolist()}") # DEBUG
    
    # Define the final list of features the model will be trained on.
    # **Ensure this uses 'transaction_value' to match your database column.**
    feature_columns = ['transaction_value'] + [col for col in df_encoded.columns if isinstance(col, str) and col.startswith('city_')]

...
from routes.market_simulator import market_simulator_bp
...
PRICE_MODEL_DIR = os.path.join(BACKEND_DIR, 'price_models')
...
app.register_blueprint(market_comparison_bp)
app.register_blueprint(market_simulator_bp, url="/v1")
@app.route("/")
async def index():
    print("--- SERVER ALIVE: Main index route '/' was hit. ---")
    return jsonify({"message": "Main Server is Running"})

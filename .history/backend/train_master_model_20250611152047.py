...
async def fetch_and_prepare_data(supabase_client: Client, granularity: str):
    ...
    df.drop(columns=['transaction_date'], inplace=True) # Drop the old text column
    ...
    feature_columns = ['transaction_value'] + [col for col in df_encoded if col.startswith('city_')]
